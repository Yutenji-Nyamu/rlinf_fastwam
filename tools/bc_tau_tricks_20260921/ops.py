...
ST=Path('/data/chenyiteng/deployment-20260921/bc-tau-tricks')
...
def launch(key):
 p=checked();s=p['runs'][key];rt=Path(s['run'])/'runtime';assert not gpu_pids(s['gpus']) or key=='sarm';assert not any(a.get('ray_namespace')==s['namespace'] and a.get('state')!='DEAD' for a in actors())
