...
ST=Path('/data/chenyiteng/deployment-20260923/rlt-two-tasks/turn')
...
def stop_old(which):
 p=checked();s=p[which];ident=s['identity'];aa=[a for a in actors() if a.get('ray_namespace')==s['namespace'] and a.get('state')!='DEAD'];roots={a['pid'] for a in aa if a.get('pid')}|{ident['pid']}
 def belongs(pid):
  for _ in range(24):
  ...
  return False
 assert all(belongs(pid) and proc(pid)['uid']==1003 for pid in gpu_pids(s['gpus']))
 save(ST/(which+'-stop-attempt.json'),{'time':now(),'identity':ident,'actors':aa})
 if same(ident):os.kill(ident['pid'],signal.SIGTERM)
 for _ in range(180):
  if not same(ident) and not gpu_pids(s['gpus']):break
  time.sleep(1)
 else:raise RuntimeError('Old GPU cleanup incomplete: '+which)
 save(ST/(which+'-stopped.json'),{'time':now(),'run':s['run']})
