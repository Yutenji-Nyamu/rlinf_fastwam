...
ST=Path('/data/chenyiteng/deployment-20260920/pi05-rlt-trick-ablation')
...
def pipeline():
 ...
 qs={k:launch(k) for k in ('drop','anneal')};save(ST/'formal-dispatched.json',{'time':now(),'drivers':{k:proc(q.pid) for k,q in qs.items()}})
