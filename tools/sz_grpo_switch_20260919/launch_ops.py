...
def check(base, launching=False):
    ...
    required = {"env.train.total_num_envs": 64, "env.train.rollout_epoch": 2,
                ...
                "algorithm.update_epoch": 2, "runner.max_steps": 300, "rollout.seed": 42}
    ...
    assert values[prefix + "temperature_local"] in (1.5, 2.0, 3.0, 4.0)
    ...
    assert values.get("runner.resume_dir") == contract["resume_checkpoint"] and values.get("runner.ckpt_path") is None
    checkpoint = Path(contract["resume_checkpoint"])
    assert checkpoint.name == "global_step_" + str(contract["resume_step"])
    for rank in range(2):
        sidecar = read(checkpoint / "actor" / f"dvac_state_rank{rank:04d}.json")
        assert sidecar["runner_step"] == contract["resume_step"]
        assert sidecar["actor_rank"] == rank and sidecar["actor_world_size"] == 2
...
def status(base, require_round=False):
    ...
    result["passed"] = (result["driver_alive"] and len(alive) == 15 and result["all_actor_uid_valid"]
                         ...
                         and result["rollout_seeds_verified"] and any(row["step"] >= contract["resume_step"] for row in successes)
