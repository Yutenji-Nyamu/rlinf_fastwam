...
ST=Path('/data/chenyiteng/deployment-20260920/pi05-rlt-notricks-four')
...
def pipeline():
 ...
 qs={k:launch(k) for k in ('t05','t1','t15','t3')};save(ST/'formal-dispatched.json',{'time':now(),'drivers':{k:proc(q.pid) for k,q in qs.items()}})
...
if __name__=='__main__' and sys.argv[1]=='supervise':
 q=launch(sys.argv[2]);wait_child(sys.argv[2],q)
