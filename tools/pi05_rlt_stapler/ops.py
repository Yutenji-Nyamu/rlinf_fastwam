...
ST=Path('/data/chenyiteng/deployment-20260919/pi05-rlt-stapler-clean4')
...
def pipeline():
 ...
 env=read(Path(p['runs']['stage1-full']['run'])/'runtime/environment.json');env['HF_ENDPOINT']='https://hf-mirror.com'
 with (ST/'prepare-data.log').open('x') as f:subprocess.run([p['python'],'-u','-B',str(Path(p['ops']).with_name('dataset.py'))],env=env,stdout=f,stderr=subprocess.STDOUT,check=True)
 ...
 qs={k:launch(k) for k in ('clean',)};save(ST/'formal-dispatched.json',{'time':now(),'drivers':{k:proc(q.pid) for k,q in qs.items()}})
