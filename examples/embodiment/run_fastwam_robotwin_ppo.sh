...
# Fast-WAM PPO launcher for the verified AutoDL layout. PPO keeps a dedicated
# entrypoint so algorithm selection is explicit while retaining the same
# resource monitor and reproducibility artifacts as the GRPO runs.
...
case "$mode" in
  smoke)
    default_config=robotwin_move_stapler_pad_ppo_fastwam_a800_2gpu_smoke
    ;;
  train|formal)
    default_config=robotwin_move_stapler_pad_ppo_fastwam_a800_2gpu
    ;;
  *)
    echo "usage: $0 [smoke|train|formal] [config-name]" >&2
    exit 2
    ;;
esac
...
# Fail before Ray allocation if the full PPO Hydra contract cannot resolve.
"$PYTHON" "$src" \
  --config-path "$EMBODIED_PATH/config/" \
  --config-name "$config" \
