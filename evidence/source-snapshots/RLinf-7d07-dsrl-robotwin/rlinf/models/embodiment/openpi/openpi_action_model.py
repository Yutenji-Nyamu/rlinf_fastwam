...
def preprocess_dsrl_images(images):
    """Map raw or compact main-camera images to ``[B,1,3,64,64]`` BF16 input."""
    ...
    if main_image.ndim != 4:
        raise ValueError(
            f"DSRL main image must be BCHW or BHWC, got {tuple(main_image.shape)}"
        )
