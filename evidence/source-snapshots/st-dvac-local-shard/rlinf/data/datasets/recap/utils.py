...
try:  # lerobot >= 0.2 layout
    from lerobot.datasets.utils import [REDACTED]
except ModuleNotFoundError:  # lerobot < 0.2
    from lerobot.common.datasets.utils import [REDACTED]
...
def cast_image_features(hf_dataset):
    ...
    if needs_cast:
        ...
        hf_dataset.set_transform([REDACTED])

    return hf_dataset


def decode_image_struct_batch(batch: dict) -> dict:
    """Decode Image-feature struct dicts before ``[REDACTED]``."""
