...
class Point:
    ...
    def __init__(
        ...
        if assets_path:
            filename = Path(assets_path) / f"assets/objects/cube/textured.obj"
        builder.add_visual_from_file(filename=str(filename), scale=[size, size, size])
...
def add_robot_visual_box(task, pose: sapien.Pose | list, name: str = "box"):
    ...
    assets_path = os.getenv("ASSETS_PATH")
    if assets_path:
        box_path = Path(assets_path) / "assets/objects/cube/textured.obj"
