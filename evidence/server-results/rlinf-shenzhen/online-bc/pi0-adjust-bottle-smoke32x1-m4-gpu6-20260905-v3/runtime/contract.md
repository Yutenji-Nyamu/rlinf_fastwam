...
11:07补充：v2首轮采集后因SFT图像BF16与增强FP32网格不匹配退出。v3仅在本分支SFT模型内部把图像恢复FP32后进入原生增强，保持权重BF16、渲染、M4、采样与更新合同；从同一原始SFT新起，旧成功池不带入。新增真实增强回归测试，无共享依赖修改。
...
输出：`/data/chenyiteng/results/rlinf-shenzhen/online-bc/pi0-adjust-bottle-smoke32x1-m4-gpu6-20260905-v3`。存在则拒绝启动。wrapper独立PGID，连接既有Ray 6389，RLinf独立namespace，显式代码工作树和绝对输出路径；ASSETS_PATH指向RoboTwin仓库根，不是assets子目录；不设全局CUDA_VISIBLE_DEVICES重编号，不加载Fast原生shim。
