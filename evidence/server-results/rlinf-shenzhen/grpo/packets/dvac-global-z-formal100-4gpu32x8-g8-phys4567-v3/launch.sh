...
NAME=dvac-global-z-formal100-4gpu32x8-g8-phys4567-v3
...
if nvidia-smi -i 4,5,6,7 --query-compute-apps=pid --format=csv,noheader,nounits | grep -Eq '^[[:space:]]*[0-9]+'; then
  printf '%s\n' 'physical GPUs 4,5,6,7 are not idle' >&2
  exit 1
fi
...
printf 'MARKER=SZ_CURRENT_DVAC_GRPO_FORMAL100_PHYS4567_LAUNCHED\n'
