...
class Checkpoint(Stateful):
    ...
    def _restore_local_model_state_dict(self, model_state_dict):
        """Rebuild FSDP2 DTensors from rank-local tensor payloads."""
        ...
        for key, value in model_state_dict.items():
            ...
            if isinstance(target, DTensor) and not isinstance(value, DTensor):
                ...
                if value.shape != target_local.shape:
                    raise ValueError(
                        f"Local checkpoint shard shape mismatch for {key!r}: "
                        f"checkpoint={tuple(value.shape)}, "
                        f"expected={tuple(target_local.shape)}"
                    )
                restored_state_dict[key] = DTensor.from_local(
                    value.to(device=target_local.device, dtype=target_local.dtype),
