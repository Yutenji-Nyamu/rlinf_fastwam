...
NAME=fastwam-grpo-smoke1-current-2gpu32x1-g8-b256-u1-m10-phys23-dcp-v3-roundtrip
RUN="$ROOT/runs/$NAME"
...
args=(
  ...
  runner.max_epochs=1000 runner.max_steps=1 runner.val_check_interval=100 runner.save_interval=1 runner.resume_dir=null
...
assert cfg["runner"]["val_check_interval"] == 100
...
common=(
  ...
  --config-name robotwin_move_stapler_pad_grpo_fastwam
  'cluster.component_placement={actor\, env\, rollout:"2,3"}'
  runner.max_epochs=1000 runner.max_steps=1 runner.val_check_interval=100 runner.save_interval=1
  env.train.total_num_envs=32 env.train.rollout_epoch=1
  env.train.max_episode_steps=192 env.train.max_steps_per_rollout_epoch=192 env.train.video_cfg.save_video=false
