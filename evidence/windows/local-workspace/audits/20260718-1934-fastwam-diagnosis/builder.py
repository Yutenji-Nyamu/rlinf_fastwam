...
def compose_official_robotwin_cfg(cfg: DictConfig) -> DictConfig:
    ...
    config_dir = Path(
        _cfg_get(fastwam_cfg, "config_dir", None) or _default_fastwam_config_dir()
    ).expanduser().resolve()
...
def _validate_official_contract(model, processor, cfg: DictConfig, official_cfg) -> None:
...
def build_fastwam_policy(
    ...
    checkpoint_path = Path(
        os.path.expandvars(
            os.path.expanduser(str(checkpoint_value))
        )
    ).resolve()
    stats_path = Path(
        os.path.expandvars(
            os.path.expanduser(str(stats_value))
        )
    ).resolve()
    ...
    policy_cfg = FastWAMPolicyConfig(
        ...
        model_forward_batch_size=int(
            _cfg_get(cfg, "model_forward_batch_size", 2)
        ),
        eval_seed=int(_cfg_get(cfg, "eval_seed", 0)),
        noise_level=float(_cfg_get(_cfg_get(cfg, "rl", {}), "noise_level", 0.1)),
    )
    policy = FastWAMPolicy(model=model, processor=processor, config=policy_cfg)
    logger.info(
