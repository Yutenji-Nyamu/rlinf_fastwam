...
def flow_step_mean_std(
    *,
    ...
    next_t = _right_broadcast(
        _expand_batch_vector(next_normalized_timestep.float(), batch_size).to(
            x.device
        ),
        x,
    )
...
def flow_sde_rollout(
    ...
    *,
    ...
    if deterministic:
        ...
        denoise_inds = denoise_inds.to(
            device=_model_device(model), dtype=torch.long
        )
        if not bool(
            ((denoise_inds >= 0) & (denoise_inds < num_inference_steps)).all()
        ):
    ...
    for step_index in range(num_inference_steps):
        ...
        mean, std, mean_ode = flow_step_mean_std(
            ...
            next_normalized_timestep=schedule.next_normalized_timesteps[
                step_index
            ],
        ...
        selected_logprob = torch.where(
            selected_view, sampled_logprob, selected_logprob
        )
...
def recompute_logprob(
    ...
    *,
    ...
    return gaussian_logprob(realized_next, mean, std), gaussian_entropy(std)
