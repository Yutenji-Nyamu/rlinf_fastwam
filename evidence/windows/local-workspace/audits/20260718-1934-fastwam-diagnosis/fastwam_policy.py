...
class FastWAMPolicy(nn.Module, BasePolicy):
    ...
    def __init__(self, *, model: nn.Module, processor: Any, config: FastWAMPolicyConfig):
        super().__init__()
        self.model = model
    ...
    def _validate_config(self) -> None:
        ...
        if cfg.rand_device != "cpu":
            raise ValueError("Pinned official Fast-WAM action noise is generated on CPU")
        if cfg.tiled:
            raise ValueError("Pinned official WanVideoVAE does not support tiled encoding")
    ...
    def predict_action_batch(
        ...
        **kwargs,
        ...
        if compute_values:
            raise NotImplementedError("First-version Fast-WAM GRPO is critic-free")
        ...
        for start in range(0, batch_size, chunk_size):
            ...
            if not deterministic:
                if (
                    ...
                    raise RuntimeError("Stochastic Fast-WAM rollout omitted replay data")
        ...
        replay = {
            ...
            "text_context_mask": _cpu_detached(
                text_context_mask, dtype=torch.bool
            ),
    ...
    def default_forward(
        ...
        **kwargs,
        ...
        for start in range(0, batch_size, chunk_size):
            stop = min(start + chunk_size, batch_size)
            item = slice(start, stop)
            logprob, entropy = recompute_logprob(
                self.model,
            ...
            logprobs.append(
                logprob[:, : self.config.num_action_chunks].float()
            )
            if compute_entropy:
                entropies.append(
                    entropy[:, : self.config.num_action_chunks].float()
                )
