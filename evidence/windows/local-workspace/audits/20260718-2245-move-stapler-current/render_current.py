...
snapshot_time = resources[-1]["timestamp"].strftime("%H:%M CST")  # type: ignore[union-attr]
html = f'''<div id="fastwamMoveStaplerCurrent">
  <div class="viz-grid">
    <div class="card viz-stat"><span class="text-muted">完成进度</span><span class="viz-stat-value">{steps[-1]} / 100</span><span class="text-small text-muted">{snapshot_time}；下一步运行中</span></div>
...
<style>
  #fastwamMoveStaplerCurrent {{ display:grid; gap:16px; color:var(--foreground); }}
  #fastwamMoveStaplerCurrent section {{ min-width:0; }}
  #fastwamMoveStaplerCurrent h3 {{ margin:0 0 6px; }}
  #fastwamMoveStaplerCurrent .fw-chart {{ position:relative; width:100%; min-width:0; height:260px; }}
  @media (max-width:480px) {{ #fastwamMoveStaplerCurrent .fw-chart {{ height:230px; }} }}
</style>
<script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.7/dist/chart.umd.min.js"></script>
<script>
(() => {{
  const root = document.getElementById('fastwamMoveStaplerCurrent');
