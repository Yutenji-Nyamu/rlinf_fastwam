...
class ActionConditioning:
    ...
    video_seq_len: int
    observation_value_features: torch.Tensor | None = None
...
def _pool_observation_value_features(
    video_kv_cache: list[dict[str, torch.Tensor]],
    *,
    batch_size: int,
    video_seq_len: int,
    ...
    """Pool the last official Fast-WAM video-cache value tensor.

    Upstream ``MoT.prefill_video_cache`` defines every cached ``v`` as
    ``[B, Sv, H * Dh]``.  The final cache is the deepest observation-only
    representation exposed by that public path: it has traversed all earlier
    video blocks and their text/proprio cross-attention, while remaining
    independent of action latents and denoise noise.
    """

    if not video_kv_cache:
        raise ValueError("Fast-WAM video KV cache must contain at least one layer")
    final_cache = video_kv_cache[-1]
    if not isinstance(final_cache, dict) or "v" not in final_cache:
        raise ValueError("Final Fast-WAM video cache must contain `v`")
    values = final_cache["v"]
    if not torch.is_tensor(values) or values.ndim != 3:
        shape = tuple(values.shape) if torch.is_tensor(values) else type(values)
        raise ValueError(
            "Final Fast-WAM video cache `v` must be [B,Sv,D], " f"got {shape}"
        )
    if values.shape[0] != batch_size or values.shape[1] != video_seq_len:
        raise ValueError(
            "Final Fast-WAM video cache `v` batch/sequence mismatch: "
            f"got {tuple(values.shape[:2])}, expected "
            f"({batch_size}, {video_seq_len})"
        )
    feature_dim = int(values.shape[2])
    if feature_dim <= 0:
        raise ValueError("Final Fast-WAM video cache `v` has an empty feature axis")
    if expected_dim is not None and feature_dim != int(expected_dim):
        raise ValueError(
            "Final Fast-WAM video cache `v` feature mismatch: "
            f"got {feature_dim}, expected {int(expected_dim)}"
        )
    if not values.is_floating_point() or not bool(torch.isfinite(values).all()):
        raise FloatingPointError(
            "Final Fast-WAM video cache `v` must be floating-point and finite"
        )

    features = values.mean(dim=1)
    if features.shape != (batch_size, feature_dim):
        raise RuntimeError(
            "Pooled Fast-WAM observation value feature has an invalid shape: "
            f"{tuple(features.shape)}"
        )
    if not bool(torch.isfinite(features).all()):
        raise FloatingPointError(
            "Pooled Fast-WAM observation value feature contains NaN/Inf"
        )
    return features
...
def encode_first_frame_latents(
    ...
    *,
    ...
    """Use the official VAE wrapper for a real batch of one-frame videos."""
    ...
    videos = [
        input_image[index].to(device=_model_device(model)).unsqueeze(1)
...
def build_action_conditioning(
    ...
    *,
    ...
    action_horizon: int,
    tiled: bool = False,
    include_observation_value_features: bool = False,
    expected_value_feature_dim: int | None = None,
) -> ActionConditioning:
    """Mirror upstream ``infer_action`` through the prefetched video KV cache.

    PPO may opt in to a detached observation-side critic feature pooled from
    the final official video-cache ``v``.  GRPO leaves the option disabled and
    therefore preserves the original conditioning payload and behavior.
    """
    ...
    observation_value_features = None
    if include_observation_value_features:
        observation_value_features = _pool_observation_value_features(
            video_kv_cache,
            batch_size=batch_size,
            video_seq_len=video_seq_len,
            expected_dim=expected_value_feature_dim,
        )
    return ActionConditioning(
        ...
        video_seq_len=video_seq_len,
        observation_value_features=observation_value_features,
    )
...
def predict_action_velocity(
    ...
    *,
    ...
    return model.action_expert.post_dit(action_tokens, action_pre).float()
...
def recompute_logprob(
    ...
    *,
    ...
    noise_level: float,
    tiled: bool = False,
    return_observation_value_features: bool = False,
    expected_value_feature_dim: int | None = None,
) -> (
    ...
    conditioning = build_action_conditioning(
        ...
        tiled=tiled,
        include_observation_value_features=return_observation_value_features,
        expected_value_feature_dim=expected_value_feature_dim,
    )
    schedule = resolve_action_schedule(model, num_inference_steps, sigma_shift)
    ...
    if not return_observation_value_features:
        return logprob, entropy
    observation_value_features = conditioning.observation_value_features
    if observation_value_features is None:
        raise RuntimeError("PPO requested observation value features but got none")
    return logprob, entropy, observation_value_features
