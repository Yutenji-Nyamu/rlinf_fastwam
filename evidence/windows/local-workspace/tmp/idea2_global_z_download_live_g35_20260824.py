...
DEST = (
    ...
    "global_z_w0to2_live_g35_20260824/raw"
)

FILES = [
    (f"{REMOTE_RUN}/metrics.log", "run/metrics.log"),
    (
        f"{REMOTE_RUN}/dvac_train/actor_rank00/runner_step_metrics.csv",
        "run/dvac_train/actor_rank00/runner_step_metrics.csv",
    ),
    (
        f"{REMOTE_RUN}/dvac_train/actor_rank01/runner_step_metrics.csv",
        "run/dvac_train/actor_rank01/runner_step_metrics.csv",
    ),
    (
        f"{REMOTE_RUN}/dvac_train/actor_rank00/rolling_stats_state.json",
        "run/dvac_train/actor_rank00/rolling_stats_state.json",
    ),
    (
        f"{REMOTE_RUN}/dvac_train/actor_rank01/rolling_stats_state.json",
        "run/dvac_train/actor_rank01/rolling_stats_state.json",
    ),
    (
        f"{REMOTE_RUN}/dvac_train/actor_rank00/run_manifest.json",
        "run/dvac_train/actor_rank00/run_manifest.json",
    ),
    (
        f"{REMOTE_RUN}/dvac_train/actor_rank01/run_manifest.json",
        "run/dvac_train/actor_rank01/run_manifest.json",
    ),
    (
        f"{REMOTE_RUN}/dvac_train/actor_rank00/rollout_step0034.npz",
        "run/dvac_train/actor_rank00/rollout_step0034.npz",
    ),
    (
        f"{REMOTE_RUN}/dvac_train/actor_rank01/rollout_step0034.npz",
        "run/dvac_train/actor_rank01/rollout_step0034.npz",
    ),
            ...
            for remote, rel in FILES:
                ...
                if rel in GROWING:
                    if actual < attrs.st_size:
                        raise RuntimeError(
                            f"truncated growing file {rel}: {actual} < {attrs.st_size}"
                        )
                elif actual != attrs.st_size:
                    raise RuntimeError(
                        f"size mismatch for {rel}: {actual} != {attrs.st_size}"
                    )
