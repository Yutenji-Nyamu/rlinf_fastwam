...
def update_obs(observation):
    ...
    # Preserve end-effector observations for policies that need the Robotwin
    # eef state. Existing qpos policies keep using `state` above.
    endpose = observation.get("endpose", None)
    if isinstance(endpose, dict):
        try:
            out["endpose_state"] = np.concatenate(
                [
...
class SubEnv:
    def __init__(
        ...
        self.action_type = str(self.args.get("action_type", "qpos")).lower()
        if self.action_type not in {"qpos", "ee"}:
            raise ValueError(f"Unsupported Robotwin action_type={self.action_type!r}; expected 'qpos' or 'ee'.")
    ...
    def reset(self, env_seed=None):
        with self.global_lock:
            with self.lock:
                ...
                control_trace = self.args.get("control_trace", {})
                if bool(control_trace.get("enabled", False)):
                    # setup_task() is only a validity probe.  The first real
                    # reset is episode zero for deterministic trace sampling.
                    control_trace["episode_index_within_slot"] = self.reset_count - 1
...
class VectorEnv(gym.Env):
    def __init__(
        ...
        args["n_envs"] = n_envs
        args["action_type"] = str(args.get("action_type", "qpos")).lower()
        if args["action_type"] not in {"qpos", "ee"}:
            raise ValueError(f"Unsupported Robotwin action_type={args['action_type']!r}; expected 'qpos' or 'ee'.")
        args["action_dim"] = 16 if args["action_type"] == "ee" else 14
    ...
    def _init_envs(self):
        for i in range(self.n_envs):
            sub_env_args = self.args
            control_trace = self.args.get("control_trace", {})
            if bool(control_trace.get("enabled", False)):
                # Only the opt-in recorder needs per-slot task arguments.
                # Keep the default path byte-for-byte equivalent.
                sub_env_args = deepcopy(self.args)
                sub_env_args["control_trace"]["env_slot"] = i
            sub_env = SubEnv(
                env_id=i,
