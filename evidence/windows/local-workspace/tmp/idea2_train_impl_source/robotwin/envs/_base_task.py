...
from .control_trace import ControlTraceRecorder
...
class Base_Task(gym.Env):
    ...
    def _init_task_env_(self, table_xy_bias=[0, 0], table_height_bias=0, **kwags):
        ...
        self._control_trace_query_idx = 0
        self._control_trace_physics_idx = 0
        ...
        self._control_trace_query_idx = 0
        self._control_trace_physics_idx = 0
        self.control_trace = ControlTraceRecorder.from_config(
            kwags.get("control_trace"),
            reset_id=kwags.get("now_ep_num", kwags.get("seed", 0)),
    ...
    def close_env(self, clear_cache=False):
        if getattr(self, "control_trace", None) is not None:
            self._finish_control_trace("environment_close")
    ...
    def _start_control_trace_query(self, action_slot_start, chunk_len):
        query_idx = self._control_trace_query_idx
        self._control_trace_query_idx += 1
        if self.control_trace is not None:
            self.control_trace.start_query(
                query_idx=query_idx,
    ...
    def _record_control_trace_step(
        ...
        *,
        ...
        physics_idx = self._control_trace_physics_idx
        self._control_trace_physics_idx += 1
        if self.control_trace is None:
            return

        row = self.control_trace.select_frame(
            control_idx=control_idx,
            physics_idx=physics_idx,
            left_control_idx=left_control_idx,
            left_control_len=left_control_len,
            right_control_idx=right_control_idx,
            right_control_len=right_control_len,
        ...
        try:
            # _update_render() already ran for this physics step.  Only read
            # the selected head frame; do not advance simulation or RNG.
            self.cameras.update_picture()
            frame = self.cameras.get_rgb()[self.control_trace.camera]["rgb"]
            self.control_trace.write_frame(frame, row)
        except Exception as exc:
            print(f"Control trace disabled after recording error: {type(exc).__name__}: {exc}")
            self.control_trace.finish(f"recording_error:{type(exc).__name__}")
            self.control_trace = None

    def _finish_control_trace(self, reason):
        recorder = self.control_trace
        self.control_trace = None
        if recorder is not None:
            try:
                ...
                print(
                    "Control trace close failed without affecting the environment: "
    ...
    def gen_sparse_reward_data(self, chunk_actions, action_type="qpos"):  # action_type: qpos or ee
        ...
        if getattr(self, "eval_success", False):
            self._finish_control_trace("success_before_query")
            infos["success"] = True
        ...
        if self.take_action_cnt >= self.step_lim:
            self._finish_control_trace("step_limit_before_query")
            truncation = np.array([1], dtype=np.int32)
            return reward, termination, truncation, infos

        action_type = str(action_type or "qpos").lower()
        if action_type not in {"qpos", "ee"}:
            raise ValueError(f"Unsupported action_type={action_type!r}; expected 'qpos' or 'ee'.")
        ...
        if action_type == "ee":
            ...
            if actions.ndim != 2 or actions.shape[-1] != 16:
                raise ValueError(
                    f"Robotwin ee chunk actions must have shape [T,16], got {actions.shape}."
                )
            for step_idx, action in enumerate(actions):
                if self.take_action_cnt >= self.step_lim or getattr(self, "eval_success", False):
                    break
                self.take_action(action, action_type="ee")
                if getattr(self, "eval_success", False):
                    infos["success"] = True
                    infos["n_steps_to_run"] = np.array([step_idx], dtype=np.int32)
                    reward = np.array([1], dtype=np.float32)
                    termination = np.array([1], dtype=np.int32)
                    return reward, termination, truncation, infos
            if getattr(self, "eval_success", False):
        ...
        action_slot_start = int(self.take_action_cnt)
        chunk_len = int(chunk_actions.shape[0])
        if self.control_trace is not None:
            self._start_control_trace_query(action_slot_start, chunk_len)
        self.take_action_cnt += chunk_len
        ...
        while now_left_id < left_n_step or now_right_id < right_n_step:
            ...
            success_after = bool(self.check_success())
            query_end = (
                now_left_id >= left_n_step and now_right_id >= right_n_step
            )
            if self.control_trace is not None:
                self._record_control_trace_step(
                    control_idx=steps_executed - 1,
                    left_control_idx=now_left_id,
                    left_control_len=left_n_step,
                    right_control_idx=now_right_id,
                    right_control_len=right_n_step,
                    success_after=success_after,
                    query_end=query_end,
                )
            ...
            if success_after:
                ...
                reward = np.array([1], dtype=np.float32)
                termination = np.array([1], dtype=np.int32)
                self._finish_control_trace("success")
                return reward, termination, truncation, infos
        ...
        if self.take_action_cnt >= self.step_lim:
            truncation = np.array([1], dtype=np.int32)
            self._finish_control_trace("step_limit")
