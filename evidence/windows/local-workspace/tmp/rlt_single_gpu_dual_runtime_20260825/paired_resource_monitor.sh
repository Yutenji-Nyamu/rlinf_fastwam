...
while true; do
  ...
  control_driver_rss=$(pgid_rss "$control_driver_pgid")
  control_head_rss=$(pgid_rss "$control_head_pgid")
  dvac_driver_rss=$(pgid_rss "$dvac_driver_pgid")
  dvac_head_rss=$(pgid_rss "$dvac_head_pgid")
  compute_count=$(nvidia-smi --query-compute-apps=pid --format=csv,noheader,nounits 2>/dev/null | awk 'NF {n++} END {print n+0}')
  printf '%s\n' "$now,$current,$high,$max,$event_high,$event_max,$event_oom,$event_oom_kill,$anon,$file,$psi_some,$psi_full,$gpu0_used,$gpu0_util,$gpu1_used,$gpu1_util,$control_driver_rss,$control_head_rss,$dvac_driver_rss,$dvac_head_rss,$compute_count" >>"$out"
