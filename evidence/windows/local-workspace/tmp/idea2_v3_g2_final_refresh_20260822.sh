...
grep -aE 'Generating Rollout Epochs|Global Step' "$runtime/driver.log" 2>/dev/null | tail -n 8 || true
...
printf 'NPZ_COUNT=%s CSV_COUNT=%s MP4_COUNT=%s CHECKPOINT_DIRS=%s\n' \
  "$(find "$run" -type f -name '*.npz' | wc -l)" \
  "$(find "$run" -type f -name '*.csv' | wc -l)" \
  "$(find "$run" -type f -name '*.mp4' | wc -l)" \
  "$(find "$run" -maxdepth 2 -type d -name 'global_step_*' | wc -l)"
printf 'GPU\n'
