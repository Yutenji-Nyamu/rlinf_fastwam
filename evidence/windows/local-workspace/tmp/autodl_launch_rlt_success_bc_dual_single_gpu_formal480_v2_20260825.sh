...
setsid "$ray_script" "$node_ip" "$pair_runtime" \
...
control_name=robotwin_adjust_bottle_rlt_single_gpu_control_formal480_20260825_v2
method_name=robotwin_adjust_bottle_rlt_single_gpu_success_episode_bc_dvac_formal480_20260825_v2
...
setsid "$driver_script" 0 "$control_config" "$control_run" "$control_runtime" \
  "$control_name" "$ray_address" rlt_control_f480_v2 \
...
setsid bash -c 'sleep 120; exec "$@"' _ "$driver_script" 1 "$method_config" \
  "$method_run" "$method_runtime" "$method_name" "$ray_address" rlt_method_f480_v2 \
