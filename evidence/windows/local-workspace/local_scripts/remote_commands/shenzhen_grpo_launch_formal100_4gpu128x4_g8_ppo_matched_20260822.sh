...
nohup setsid "$VENV/bin/python" "$WT/examples/embodiment/train_embodied_agent.py" \
  "${ARGS[@]}" > "$RUN/driver.log" 2>&1 < /dev/null &
pid=$!
printf '%s\n' "$pid" > "$RUN/driver.pid"
nohup setsid bash "$RUN/resource_observer.sh" "$pid" "$RUN/resource.csv" \
...
printf '%s\n' 'SZ_GRPO_FORMAL100_PPO_MATCHED_LAUNCHED'
