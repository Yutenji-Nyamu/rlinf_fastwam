...
RUN=/data/chenyiteng/results/rlinf-shenzhen/ppo/ppo-reload-fixed64-4gpu4567-v1
...
if nvidia-smi -i 4,5,6,7 --query-compute-apps=pid --format=csv,noheader,nounits \
  ...
  printf '%s\n' 'GPU compute process already present on physical GPU 4-7; not starting reload eval' >&2
  exit 1
fi
