...
mkdir -p "$CHAIN"
nvidia-smi -i 4,5,6,7 --query-compute-apps=gpu_uuid,pid,used_memory,process_name --format=csv,noheader,nounits > "$CHAIN/gpu4_7_before.txt"

cat > "$CHAIN/worker.sh" <<'WORKER'
#!/usr/bin/env bash
set -euo pipefail
WT=/data/chenyiteng/projects/rlinf-shenzhen/worktrees/pi05-robotwin-rl
VENV=/home/chenyiteng/venvs/rlinf-7d07-openpi-robotwin
...
run_one() {
  local name=$1 packet_name=$2 steps=$3 experiment=$4 kind=$5
  ...
  test "$rc" -eq 0
  local actor_ckpt="$run/$experiment/checkpoints/global_step_$steps/actor"
  test -s "$actor_ckpt/local_shard_checkpoint/checkpoint_rank_0.pt"
  test -s "$actor_ckpt/local_shard_checkpoint/checkpoint_rank_1.pt"
  if test "$kind" = dvac; then
    test "$(find "$actor_ckpt" -maxdepth 1 -type f -name 'dvac_state_rank*.json' | wc -l)" -eq 2
  fi
...
printf 'PI05_GRPO_SMOKES_OK\n'
WORKER
chmod 700 "$CHAIN/worker.sh"
nohup setsid bash "$CHAIN/worker.sh" > "$CHAIN/chain.log" 2>&1 < /dev/null &
pid=$!
printf '%s\n' "$pid" > "$CHAIN/worker.pid"
sleep 15
kill -0 "$pid"
printf 'chain=%s\npid=%s\nPI05_GRPO_SMOKES_LAUNCHED\n' "$CHAIN" "$pid"
