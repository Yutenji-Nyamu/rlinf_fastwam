...
NAME=prism-dvac-rank-rloo-smoke1-2gpu64x4-b1024-noeval-phys23-v1
RUN="$ROOT/runs/$NAME"
...
args=(
  ...
  runner.logger.experiment_name=robotwin_prism_dvac_rank_rloo_smoke1_2gpu64x4_b1024_noeval_phys23_v1
...
printf '%s\n' \
  ...
  'method=mean log V_L3 -> G8 reverse rank q -> success+0.2q -> RLOO/no-std' \
  'eval=disabled only for smoke; terminal checkpoint global_step_1' \
  'normal_stop=complete step1/checkpoint/exit0; hard_timeout=7200s' \
...
nohup setsid bash "$RUN/runtime/observer.sh" "$pid" "$RUN/runtime/resource.csv" \
...
echo SZ_PRISM_DVAC_RANK_RLOO_SMOKE1_LAUNCHED
