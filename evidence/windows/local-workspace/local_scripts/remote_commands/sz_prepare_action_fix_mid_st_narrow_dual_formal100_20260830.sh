...
compose_action "$ACTION_PACKET/resolved.yaml" action_level apply action_advantage 0.5 1.5 robotwin_dvac_action_adv_fix_w0p5to1p5_formal100_2gpu64x4_b1024_fixed32_eval5_phys45_v1

use_source "$ST_WT"
compose_st "$ST_PACKET/control_same_code_resolved.yaml" off null null robotwin_st_half_same_code_control_not_run
compose_st "$ST_PACKET/resolved.yaml" apply 0.8 1.2 robotwin_dvac_st_global_z_w0p8to1p2_formal100_2gpu64x4_b1024_fixed32_eval5_phys67_v1
...
check_budget(action, "4,5", "action_level", "apply", 0.5, 1.5)
...
check_budget(st, "6,7", "chunk_level", "apply", 0.8, 1.2)
...
outputs = (
    (action_output, {
        ...
        "method": "GRPO DVAC Action-Adv Fix [0.5,1.5]: action-level ratio/clip; A_eff=A*w; sum valid H then mean queries",
    }),
    (st_output, {
        ...
        "method": "GRPO DVAC ST global-z [0.8,1.2]: Control joint-chunk forward/clip; local backward scaling only",
    }),
)
for path, payload in outputs:
    with open(path, "w", encoding="utf-8") as handle:
        ...
        handle.write("\n")
print("SZ_ACTION_FIX_MID_AND_ST_NARROW_PACKET_PARITY_OK unexpected=0")
PY
...
cat > "$ACTION_PACKET/contract.json" <<EOF
{
  ...
  "method": "trajectory GRPO A; action-level ratio/clip; A_eff=A*stopgrad(DVAC w in [0.5,1.5]); fixed source sums valid H then means queries",
...
cat > "$ST_PACKET/contract.json" <<EOF
{
  ...
  "train": "64 env x 4 rollout epochs = 256 trajectories/step; G8; max 1024 chunk records",
  "actor": "GB1024/MB32/update2",
  "method": "trajectory GRPO A; Control joint-chunk ratio/clip; DVAC ST global-z local gradient multipliers in [0.8,1.2]",
  "formal": "fresh 100 steps; fixed32 every5; default DCP checkpoint every10",
  "normal_stop": "complete global_step_100 and exit0",
  "hard_timeout_seconds": 216000
}
...
echo SZ_ACTION_FIX_MID_AND_ST_NARROW_DUAL_FORMAL100_PACKETS_READY
