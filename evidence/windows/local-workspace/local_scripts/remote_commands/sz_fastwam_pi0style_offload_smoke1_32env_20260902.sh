...
SLUG=fastwam-grpo-pi0style-offload-smoke1-2gpu32x8-g8-b2048-u2-m10-phys67-v1
...
EXP=fastwam_grpo_pi0style_offload_smoke1_2gpu32x8_g8_b2048_u2_m10_phys67_v1
...
assert b['env.train.total_num_envs']==32
assert b['env.train.rollout_epoch']==8
...
cat > "$PACKET/contract.json" <<EOF
{
  ...
  "fallback_only_on_oom": "16 env x rollout16"
}
EOF
...
timeout --signal=TERM --kill-after=180s 21600s bash -lc "$(cat "$runtime/command.txt")" > "$runtime/driver.log" 2>&1
rc=$?
printf '%s\n' "$rc" > "$runtime/exit_code.txt"
date --iso-8601=seconds > "$runtime/finished_at.txt"
exit "$rc"
WRAP
chmod +x "$RUN/runtime/wrapper.sh"
...
OBS
chmod +x "$RUN/runtime/observer.sh"
...
purpose=one-step resource smoke; not formal
physical_gpus=6,7
sampling=32 env x rollout8 = 256 trajectories; G8; max2048 query records
...
fallback=only on observed OOM, 16 env x rollout16 with 256 trajectories and 2048 records unchanged
EOF
