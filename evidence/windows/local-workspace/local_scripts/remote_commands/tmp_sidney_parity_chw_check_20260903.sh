...
INPUT=/data/chenyiteng/results/rlinf-shenzhen/sidney-pi05-parity-preflight/input.npz
...
with np.load(artifact, allow_pickle=False) as raw:
    batch = preprocessor({
        ...
        'observation.images.cam_high': torch.from_numpy(raw['main_image']).permute(2,0,1).float()/255.0,
        'observation.images.cam_left_wrist': torch.from_numpy(raw['wrist_images'][0]).permute(2,0,1).float()/255.0,
        'observation.images.cam_right_wrist': torch.from_numpy(raw['wrist_images'][1]).permute(2,0,1).float()/255.0,
        'task': str(raw['prompt'].item()),
    })
shapes = {key: tuple(batch[key].shape) for key in config.image_features}
assert set(shapes.values()) == {(1,3,480,640)}, shapes
images, _ = ImageAdapter(config)._preprocess_images(batch)
model_shapes = [tuple(image.shape) for image in images]
assert model_shapes == [(1,3,224,224)] * 3, model_shapes
print('native_raw_processor_chw_ok', shapes)
print('native_model_images_chw_ok', model_shapes)
PY
cd "$WT"
git diff --check
git diff --stat
