...
OLD=/data/chenyiteng/results/rlinf-shenzhen/grpo/runs/dvac-global-z-w0to2-formal100-2gpu64x4-b1024-fixed32-eval5-phys67-v2
...
test ! -e "$RUN"
for file in resolved.yaml control_same_code_resolved.yaml parity.json contract.json command.txt; do
  test -s "$PACKET/$file"
done
...
ray.init(address=os.environ["RAY_ADDRESS"], namespace="codex_cutover_to_prism", logging_level="ERROR")
...
assert targets, "expected old DVAC namespace actors"
...
cat > "$OLD/runtime/stopped_by_user_for_prism.txt" <<EOF
stopped_at=$(TZ=Asia/Shanghai date --iso-8601=seconds)
reason=user requested replacement of DVAC global-z [0,2] with Prism-style DVAC-Rank-RLOO formal
last_complete_step=$last_complete_step
ray_job_id=$OLD_JOB
namespace=$OLD_NAMESPACE
cleanup=exact owned PGID and exact named actors; Control/shared Ray/other users unchanged
EOF
...
mkdir -p "$RUN/runtime"
cp "$PACKET/resolved.yaml" "$PACKET/control_same_code_resolved.yaml" \
   "$PACKET/parity.json" "$PACKET/contract.json" "$PACKET/command.txt" \
   "$RUN/runtime/"
...
printf '%s\n' \
  ...
  'method=mean log V_L3 -> G8 reverse rank q -> success+0.2q -> RLOO/no-std; DVAC-ST off' \
  'eval=fixed32 every5; checkpoint every10' \
...
nohup setsid bash "$RUN/runtime/observer.sh" "$new_pid" "$RUN/runtime/resource.csv" \
  > "$RUN/runtime/observer.log" 2>&1 < /dev/null &
observer=$!
...
echo SZ_DVAC_W0TO2_TO_PRISM_FORMAL100_CUTOVER_OK
