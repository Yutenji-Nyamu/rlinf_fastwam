...
for seed in $seeds; do
  ...
  mkdir -p "$run_root"
  printf 'task=%s\nseed=%s\nn_episodes=1\ngpu_physical=%s\nmodel_revision=%s\nlerobot_revision=%s\n' \
    "$task" "$seed" "$gpu" e49e2ab6c11f07511573b67261bd129e88d0a416 30da8e687a6dfc617fcd94afc367ac7071c376ce > "$run_root/run_identity.txt"
  set +e
  ...
  if [ "$rc" -ne 0 ]; then
    if grep -q 'UnStableError' "$run_root/eval.log"; then printf 'UNSTABLE_AFTER_PROBE task=%s seed=%s\n' "$task" "$seed" | tee "$run_root/result.txt"; continue; fi
