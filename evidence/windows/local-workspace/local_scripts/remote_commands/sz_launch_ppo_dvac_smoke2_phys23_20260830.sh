...
CONTROL="$ROOT/runs/ppo-control-smoke1-2gpu64x4-b1024-noeval-localshard-phys23-v1"
NAME=ppo-dvac-action-adv-fix-w0to2-smoke2-2gpu64x4-b1024-noeval-localshard-phys23-v1
RUN="$ROOT/runs/$NAME"
EXPERIMENT=robotwin_ppo_dvac_action_adv_fix_w0to2_smoke2_2gpu64x4_b1024_noeval_localshard_phys23_v1

test "$(git -C "$WT" rev-parse HEAD)" = "$HEAD"
test -z "$(git -C "$WT" status --short)"
test "$(cat "$CONTROL/runtime/exit_code.txt")" = 0
CONTROL_CKPT="$CONTROL/robotwin_ppo_control_smoke1_2gpu64x4_b1024_noeval_localshard_phys23_v1/checkpoints/global_step_1/actor"
test -s "$CONTROL_CKPT/local_shard_checkpoint/checkpoint_rank_0.pt"
test -s "$CONTROL_CKPT/local_shard_checkpoint/checkpoint_rank_1.pt"
test -s "$CONTROL_CKPT/model_state_dict/full_weights.pt"
test ! -e "$RUN"
...
import json, os, ray
ray.init(address=os.environ["RAY_ADDRESS"], namespace="codex_ppo_dvac_smoke_preflight", logging_level="ERROR")
...
print("PPO_DVAC_SMOKE_RESOLVED_OK")
PY
...
printf '%s\n' \
  ...
  'method=DVAC Action-Adv Fix [0,2]; L3; warmup1; recent5' \
  'eval=disabled only for smoke; videos=false; terminal checkpoint local_shard global_step_2' \
...
echo SZ_PPO_DVAC_SMOKE2_LAUNCHED
