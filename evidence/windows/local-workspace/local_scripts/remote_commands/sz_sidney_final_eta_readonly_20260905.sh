...
log=(root/'runtime/driver.log').read_text(errors='replace');state={k:(root/'runtime'/k).read_text().strip() if (root/'runtime'/k).exists() else None for k in ['started_at.txt','finished_at.txt','exit_code.txt','wrapper.pid']}
d={'time':datetime.datetime.now(tz).isoformat(),'scalars':scalars,'completed_steps':re.findall(r'Global Step:\s*(\d+)\s*/',log)[-1:],'state':state,'wrapper_alive':Path('/proc',state['wrapper.pid']).exists(), 'phase':[l for l in log.splitlines() if 'Generating Rollout Epochs:' in l][-3:],'errors':{k:log.count(k) for k in ['Fatal Python error','CUDA out of memory','RuntimeError:']}}
d['disk']=subprocess.run(['df','-B1','/data'],capture_output=True,text=True).stdout
d['step100_files']=[{'path':str(p),'bytes':p.stat().st_size} for p in root.glob('*/checkpoints/global_step_100/actor/**/*') if p.is_file()]
print(json.dumps(d))
PY
