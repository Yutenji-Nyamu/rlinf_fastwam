...
cd "$root"
nice -n 19 /home/chenyiteng/venvs/rlinf-7d07-openpi-robotwin/bin/python -B -m pytest -p no:cacheprovider -q tests/unit_tests/test_online_bc.py tests/unit_tests/test_pi05_online_bc.py tests/unit_tests/test_online_bc_dvac.py tests/unit_tests/test_pi05_online_bc_dvac.py
...
for name,run in runs.items():
 ...
 for path in [files[0],files[1],files[-1]]:
  ...
  item={'path':str(path),'episodes':len(eps),'queries':len(rows),'keys':list(rows[0]),
    ...
    'versions':sorted({float(r['policy_version']) for r in rows if r.get('policy_version') is not None}),
