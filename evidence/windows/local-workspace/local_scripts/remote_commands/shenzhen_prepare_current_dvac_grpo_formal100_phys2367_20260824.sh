...
printf 'MARKER=SZ_PREPARE_CURRENT_DVAC_GRPO_FORMAL100_PHYS2367_V1\n'
...
assert placement == {"actor, env, rollout": "2,3,6,7"}
assert cfg.runner.max_steps == 100
assert cfg.runner.val_check_interval == cfg.runner.save_interval == 10
assert trajectories == 256 and trajectories // cfg.algorithm.group_size == 32
assert records == 1024
assert cfg.actor.global_batch_size == 512 and cfg.actor.micro_batch_size == 32
...
print(json.dumps({
    ...
    "micro_batch": 32,
    "update_epoch": 2,
    "max_optimizer_calls_per_step": 4,
...
printf 'packet=%s\nrun=%s\nlauncher=%s\n' "$PACKET" "$RUN" "$PACKET/launch.sh"
printf 'MARKER=SZ_PREPARE_CURRENT_DVAC_GRPO_FORMAL100_PHYS2367_OK\n'
