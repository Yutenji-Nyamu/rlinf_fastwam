...
test -x "$PY"
test "$(git -C "$FW" rev-parse HEAD)" = 7faa71108368fbb3b6885649f112af607427a2d4
test "$(git -C "$CUROBO_SRC" rev-parse HEAD)" = "$CUROBO_REV"
test -d "$VENDOR/assets/objects"
test -d "$VENDOR/task_config"
test -L "$VENDOR/policy/fastwam_policy"
test -d "$PATCH_RECORD"
test ! -e "$FREEZE"
...
expected = {
    ...
    'numpy': '2.2.6', 'scipy': '1.15.3', 'setuptools': '80.9.0',
    'setuptools-scm': '9.2.2', 'transforms3d': '0.4.2',
    'sapien': '3.0.0b1', 'mplib': '0.2.1', 'gymnasium': '0.29.1',
    'trimesh': '4.4.3', 'open3d': '0.18.0', 'h5py': '3.16.0',
    'pyglet': '1.5.31', 'toppra': '0.6.3', 'opencv-python': '4.11.0.86',
    'PyYAML': '6.0.3', 'pybind11': '3.0.4', 'networkx': '3.4.2',
    'numpy-quaternion': '2024.0.13', 'yourdfpy': '0.0.60',
    'importlib-resources': '6.5.2', 'scikit-image': '0.25.2',
    'lazy-loader': '0.5', 'tifffile': '2025.3.13',
    'warp-lang': '1.11.1', 'nvidia-curobo': '0.7.8',
}
...
direct = json.loads(md.distribution('nvidia-curobo').read_text('direct_url.json'))
...
du -sh "$ENV" "$CACHE_ROOT" "$CUROBO_SRC" "$VENDOR/assets"
git -C "$FW" diff --quiet
git -C "$FW" diff --cached --quiet
test "$(git -C "$FW" status --porcelain --untracked-files=normal)" = \
