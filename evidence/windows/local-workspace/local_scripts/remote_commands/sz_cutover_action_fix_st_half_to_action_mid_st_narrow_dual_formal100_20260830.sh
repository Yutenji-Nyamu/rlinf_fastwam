...
OLD_ACTION=/data/chenyiteng/results/rlinf-shenzhen/grpo/runs/dvac-action-adv-fix-w0to2-formal100-2gpu64x4-b1024-fixed32-eval5-phys45-v1
OLD_ST=/data/chenyiteng/results/rlinf-shenzhen/grpo/runs/dvac-st-global-z-w0p5to1p5-formal100-2gpu64x4-b1024-fixed32-eval5-phys67-v1
...
ACTION_NAME=dvac-action-adv-fix-w0p5to1p5-formal100-2gpu64x4-b1024-fixed32-eval5-phys45-v1
ST_NAME=dvac-st-global-z-w0p8to1p2-formal100-2gpu64x4-b1024-fixed32-eval5-phys67-v1
...
cleanup_namespace_preserve_other() {
  local target=$1 preserve=$2 preserve_expected=$3
  RAY_ADDRESS="$RAY_ADDRESS" TARGET_NAMESPACE="$target" PRESERVE_NAMESPACE="$preserve" PRESERVE_EXPECTED="$preserve_expected" "$VENV/bin/python" - <<'PY'
import os, time, ray
...
assert len(preserved) == int(os.environ["PRESERVE_EXPECTED"]), preserved
...
start_action_fix() {
  ...
  local -a args=(
    ...
    runner.logger.experiment_name=robotwin_dvac_action_adv_fix_w0p5to1p5_formal100_2gpu64x4_b1024_fixed32_eval5_phys45_v1
  ...
  start_runtime "$ACTION_RUN" "$ACTION_PACKET" "$ACTION_HEAD" 4,5 'Action-Adv Fix [0.5,1.5]: A_eff=A*w; action-level clip; sum valid H then mean queries'
...
start_st_narrow() {
  ...
  local -a args=(
    ...
    runner.logger.experiment_name=robotwin_dvac_st_global_z_w0p8to1p2_formal100_2gpu64x4_b1024_fixed32_eval5_phys67_v1
    runner.max_epochs=1000 runner.max_steps=100 runner.val_check_interval=5 runner.save_interval=10 runner.resume_dir=null
  ...
  start_runtime "$ST_RUN" "$ST_PACKET" "$ST_HEAD" 6,7 'ST-DVAC global-z [0.8,1.2]: Control joint-chunk forward/clip; local backward scaling'
...
# The old ST run already exited after Ray memory protection; only the live Action run is stopped.
...
test "$(namespace_count "$ST_NAMESPACE")" = 0
old_st_last_step=$(grep -aoE 'Global Step:[[:space:]]+[0-9]+/100' "$OLD_ST/runtime/driver.log" | tail -n1 | grep -oE '[0-9]+' | head -n1)
test "$old_st_last_step" = 53
test "$(cat "$OLD_ST/runtime/exit_code.txt")" = 255
...
cleanup_namespace_preserve_other "$ACTION_NAMESPACE" "$ST_NAMESPACE" 0
wait_old_job_gone 4,5 "$OLD_ACTION_JOB"

cat > "$OLD_ACTION/runtime/stopped_by_user_for_action_adv_fix_mid.txt" <<EOF
stopped_at=$(TZ=Asia/Shanghai date --iso-8601=seconds)
reason=user requested replacement by Action-Adv Fix [0.5,1.5]
last_complete_step=$action_last_step
ray_job_id=$OLD_ACTION_JOB
namespace=$ACTION_NAMESPACE
cleanup=exact owned PGID and exact namespace; shared Ray/other users unchanged
EOF

cat > "$OLD_ST/runtime/archived_after_memory_exit_for_st_dvac_narrow.txt" <<EOF
archived_at=$(TZ=Asia/Shanghai date --iso-8601=seconds)
reason=existing ST-DVAC [0.5,1.5] already exited255 after Ray memory protection; replaced by fresh ST-DVAC [0.8,1.2]
last_complete_step=$old_st_last_step
namespace=$ST_NAMESPACE
cleanup=no process stop required; namespace already empty; shared Ray/other users unchanged
EOF

start_action_fix
start_observer "$ACTION_RUN" "$ACTION_PID" 4 5
wait_namespace_ready "$ACTION_NAMESPACE" "$ACTION_PID"
ACTION_NEW_JOB=$(gpu_unique_job 4,5 action_fix_mid 6)
test "$ACTION_NEW_JOB" != "$OLD_ACTION_JOB"

test "$(namespace_count "$ST_NAMESPACE")" = 0
test -z "$(nvidia-smi -i 6,7 --query-compute-apps=pid --format=csv,noheader,nounits | sed '/^[[:space:]]*$/d')"
st_cutover_epoch=$(date +%s)
start_st_narrow
start_observer "$ST_RUN" "$ST_PID" 6 7
wait_namespace_ready "$ST_NAMESPACE" "$ST_PID"
ST_NEW_JOB=$(gpu_unique_job 6,7 st_narrow 6)
test "$ACTION_NEW_JOB" != "$ST_NEW_JOB"
kill -0 "$ACTION_PID"
kill -0 "$ST_PID"

printf 'old_action_last_step=%s\nold_st_last_step=%s\n' "$action_last_step" "$old_st_last_step"
printf 'action_fix_mid_run=%s\naction_fix_mid_pid=%s\naction_fix_mid_job=%s\naction_cutover_gap_seconds=%s\n' \
  "$ACTION_RUN" "$ACTION_PID" "$ACTION_NEW_JOB" "$(( ACTION_START_EPOCH-action_cutover_epoch ))"
printf 'st_narrow_run=%s\nst_narrow_pid=%s\nst_narrow_job=%s\nst_start_gap_seconds=%s\n' \
  "$ST_RUN" "$ST_PID" "$ST_NEW_JOB" "$(( ST_START_EPOCH-st_cutover_epoch ))"
nvidia-smi -i 4,5,6,7 --query-gpu=index,memory.used,memory.total,utilization.gpu --format=csv,noheader,nounits
echo SZ_ACTION_FIX_MID_AND_ST_NARROW_DUAL_FORMAL100_CUTOVER_OK
