...
$destination = Join-Path $workspace 'docs\rlinf-shenzhen-pi0-ppo-rlt\evidence\grpo_v2_live_step36_20260823'
$connection = @(
    '--host', '120.241.223.9',
    '--port', '22',
    '--user', 'chenyiteng',
...
$files = [ordered]@{
    ...
    "$run/tensorboard/events.out.tfevents.1787412552.admin.824881.0" = 'events.out.tfevents.1787412552.admin.824881.0'
}
...
try {
    ...
    foreach ($entry in $files.GetEnumerator()) {
        ...
        if (Test-Path -LiteralPath $local) {
            throw "Refusing to overwrite local snapshot file: $local"
        }
