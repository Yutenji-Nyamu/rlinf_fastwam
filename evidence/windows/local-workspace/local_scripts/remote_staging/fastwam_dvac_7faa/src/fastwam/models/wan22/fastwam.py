...
class FastWAM(torch.nn.Module):
    ...
    def infer_action(
        ...
        tiled: bool = False,
        compile_action_infer: bool = False,
        return_action_denoising_trace: bool = False,
        action_generator: Optional[torch.Generator] = None,
    ) -> dict[str, Any]:
        self.eval()
        if return_action_denoising_trace and compile_action_infer:
            raise ValueError("Action denoising telemetry supports the official non-compiled inference path only.")
        if action_generator is not None and seed is not None:
            raise ValueError("Pass either `seed` or `action_generator`, not both.")
        ...
        if return_action_denoising_trace:
            trace_x_chain = [latents_action[0].detach().to(device="cpu", dtype=torch.float32).clone()]
            trace_v_chain = []
            trace_timesteps = []
            trace_deltas = []
            trace_x_next = []
        for step_t_action, step_delta_action in zip(infer_timesteps_action, infer_deltas_action):
            ...
            if return_action_denoising_trace:
                trace_v_chain.append(pred_action[0].detach().to(device="cpu", dtype=torch.float32).clone())
                trace_timesteps.append(step_t_action.detach().to(device="cpu", dtype=torch.float32).reshape(()).clone())
                trace_deltas.append(step_delta_action.detach().to(device="cpu", dtype=torch.float32).reshape(()).clone())
            latents_action = self.infer_action_scheduler.step(pred_action, step_delta_action, latents_action)
            if return_action_denoising_trace:
                x_next = latents_action[0].detach().to(device="cpu", dtype=torch.float32).clone()
                trace_x_next.append(x_next)
                trace_x_chain.append(x_next)

        result = {
            "action": latents_action[0].detach().to(device="cpu", dtype=torch.float32),
        }
        if return_action_denoising_trace:
            result["action_denoising_trace"] = {
                "x_chain": torch.stack(trace_x_chain, dim=0),
                "v_chain": torch.stack(trace_v_chain, dim=0),
                "timesteps": torch.stack(trace_timesteps, dim=0),
                "deltas": torch.stack(trace_deltas, dim=0),
                "x_next": torch.stack(trace_x_next, dim=0),
            }
        return result
