...
from experiments.robotwin.fastwam_policy.dvac_telemetry import FastWAMDvacTelemetryWriter
...
class WorldActionRobotWinPolicy:
    def __init__(
        ...
        dvac_writer: Optional[FastWAMDvacTelemetryWriter] = None,
    ) -> None:
        ...
        self._dvac_writer = dvac_writer
        self._pending_dvac_payload: Optional[dict[str, Any]] = None
    ...
    def _infer_action_chunk(self, observation: Dict[str, Any], instruction: str) -> np.ndarray:
        ...
        if self._dvac_writer is not None:
            infer_kwargs["return_action_denoising_trace"] = True
        ...
        if self._dvac_writer is not None:
            self._pending_dvac_payload = {
                "trace": pred["action_denoising_trace"],
                "observation": observation,
                "robot_state": state_vector,
                "final_model_action": action_tensor,
    ...
    def _fill_action_queue(self, task_env, observation: Dict[str, Any], instruction: str) -> None:
        ...
        if self._dvac_writer is not None:
            if self._pending_dvac_payload is None:
                raise RuntimeError("DVAC telemetry was enabled but the model trace is missing")
            self._dvac_writer.record_query(
                task=str(getattr(task_env, "task_name", "")),
                instruction=instruction,
                reset_id=getattr(task_env, "ep_num", ""),
                query_start_action_slot=self.step_count,
                planned_exec_length=n_exec,
                **self._pending_dvac_payload,
            )
            self._pending_dvac_payload = None
    ...
    def step(self, task_env, observation: Optional[Dict[str, Any]]) -> None:
        ...
        if self._dvac_writer is not None:
            self._dvac_writer.record_action(success_after=bool(getattr(task_env, "eval_success", False)))
        if self.timing_enabled:
    ...
    def reset(self) -> None:
        ...
        if self._dvac_writer is not None:
            self._dvac_writer.begin_episode(self.episode_count)
...
def get_model(usr_args: Dict[str, Any]):
    ...
    dvac_enabled = _parse_bool(usr_args.get("dvac_telemetry_enabled", False))
    dvac_writer = None
    if dvac_enabled:
        dvac_output_value = usr_args.get("dvac_telemetry_output_dir")
        eval_output_value = usr_args.get("eval_output_dir")
        if _is_none_like(dvac_output_value):
            if _is_none_like(eval_output_value):
                raise ValueError("DVAC telemetry requires an output directory")
            dvac_output_dir = Path(str(eval_output_value)).expanduser().resolve() / "dvac_telemetry"
        else:
            dvac_output_dir = Path(str(dvac_output_value)).expanduser().resolve()
        run_id_value = usr_args.get("dvac_telemetry_run_id")
        run_id = dvac_output_dir.name if _is_none_like(run_id_value) else str(run_id_value)
        dvac_writer = FastWAMDvacTelemetryWriter(
            dvac_output_dir,
            manifest={
                "run_id": run_id,
                "policy": "fastwam",
                "task": str(usr_args.get("task_name", "")),
                "task_config": str(usr_args.get("task_config", "")),
                "official_video_dir": (
                    None
                    if _is_none_like(eval_output_value)
                    else str(Path(str(eval_output_value)).expanduser().resolve())
                ),
                "instrumentation_base_revision": "7faa71108368fbb3b6885649f112af607427a2d4",
                "robotwin_revision": "bf44be51cf5717a5595ce59447f2cf5263d2aa95",
                "checkpoint_path": str(checkpoint_path),
                "dataset_stats_path": str(dataset_stats_path),
                "model_seed": seed,
                "H": action_horizon,
                "C": min(replan_steps, action_horizon),
                "M": num_inference_steps,
                "D": 14,
                "action_num_train_timesteps": 1000,
                "sigma_shift_override": sigma_shift,
                "normalization": "Fast-WAM dataset-stat z-score action space",
            },
