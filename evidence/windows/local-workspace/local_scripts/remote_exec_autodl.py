...
def connect(args: argparse.Namespace) -> paramiko.SSHClient:
    ...
    if not password:
        [REDACTED] SystemExit("SEETA_SSH_PASSWORD is required in the current process")
...
def run_command(client: paramiko.SSHClient, args: argparse.Namespace) -> int:
    ...
    stdin_modes = [
        ...
        bool(args.stdin_password),
        bool(args.stdin_git_diff),
        bool(args.stdin_git_full_diff),
        bool(args.stdin_git_tree_diff),
    ]
    if sum(stdin_modes) > 1:
        raise SystemExit(
            "--stdin-file, --stdin-password, --stdin-git-diff, --stdin-git-full-diff, and "
            "--stdin-git-tree-diff "
    ...
    if args.stdin_file:
        with open(args.stdin_file, "rb") as handle:
            while chunk := handle.read(1024 * 1024):
                stdin.write(chunk)
    elif args.stdin_[REDACTED]:
        [REDACTED] = os.environ.get("SEETA_SSH_PASSWORD")
        if not password:
            [REDACTED] SystemExit("SEETA_SSH_PASSWORD is required for --stdin-password")
        ...
        try:
            ...
            untracked = [path.decode("utf-8") for path in untracked if path]
            if untracked:
                subprocess.run(
                    ["git", "-C", repo, "add", "--sparse", "--", *untracked],
                    check=True,
                    env=env,
                )
            result = subprocess.run(
            ...
            if os.path.exists(index_path):
                os.unlink(index_path)
    elif args.stdin_git_tree_diff:
        if not args.stdin_git_tree_path:
            raise SystemExit("--stdin-git-tree-path is required for a tree diff")
        repo = os.path.abspath(args.stdin_git_tree_diff)
        with tempfile.NamedTemporaryFile(delete=False) as temp_index:
            index_path = temp_index.name
        os.unlink(index_path)
        env = os.environ.copy()
        ...
        try:
            subprocess.run(
                ["git", "-C", repo, "read-tree", args.stdin_git_tree_base],
                check=True,
                env=env,
            )
            subprocess.run(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run.add_argument(
        "--stdin-password",
        action="store_true",
        help="Send the process-only SSH password as one stdin line for sudo -S",
    )
    run.add_argument(
        "--stdin-git-diff",
    ...
    run.add_argument(
        "--stdin-git-tree-diff",
        help="Stream an exact path-limited diff from a pre-edit Git tree",
    )
    run.add_argument("--stdin-git-tree-base")
    run.add_argument("--stdin-git-tree-path", action="append", default=[])
