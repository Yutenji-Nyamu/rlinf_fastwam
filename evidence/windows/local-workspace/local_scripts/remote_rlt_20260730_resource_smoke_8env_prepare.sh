...
smoke_config=examples/embodiment/config/robotwin_adjust_bottle_rlt_stage2_ac_mlp_8env_resource_smoke.yaml
smoke_config_sha256=47bbdd231d53d69aad5aee35ef6c04787d6d9c7610f02df9f945961466d6acb2
...
test "$(sha256sum "$smoke_config" | awk '{print $1}')" = "$smoke_config_sha256"
...
cp "$formal_config" "$runtime_root/formal_parent_config.yaml"
cp "$smoke_config" "$runtime_root/source_config.yaml"
cp "$seed_bank" "$runtime_root/eval_seed_bank.json"
"${venv}/bin/python" -B \
  examples/embodiment/train_embodied_agent.py \
  --config-path "${repo}/examples/embodiment/config" \
  --config-name robotwin_adjust_bottle_rlt_stage2_ac_mlp_8env_resource_smoke \
...
assert cfg.runner.max_steps == 3
assert cfg.runner.val_check_interval == 3
assert cfg.runner.save_interval == 3
...
assert len(json.loads(Path(os.environ["SEED_BANK"]).read_text())["adjust_bottle"]["success_seeds"]) == 20
assert cfg.algorithm.update_epoch == 5
assert cfg.algorithm.rlt_schedule.max_updates_per_train_step == 1600
assert cfg.algorithm.rlt_schedule.warmup_min_size == 1
assert cfg.algorithm.rlt_schedule.warmup_post_collect_updates == 1600
...
timeout --signal=TERM --kill-after=180s 3600s \\
  "${venv}/bin/python" -B \\
  examples/embodiment/train_embodied_agent.py \\
  --config-path "${repo}/examples/embodiment/config" \\
  --config-name robotwin_adjust_bottle_rlt_stage2_ac_mlp_8env_resource_smoke \\
...
sleep 2
kill -0 "\$driver_pid"
printf 'SMOKE_DRIVER_PID\t%s\n' "\$driver_pid"
printf 'SMOKE_MONITOR_PID\t%s\n' "\$monitor_pid"
printf 'RUNTIME_ROOT\t%s\n' "$runtime_root"
printf 'RUN_ROOT\t%s\n' "$run_root"
printf 'EXPERIMENT_NAME\t%s\n' "$experiment_name"
...
printf 'TIMEOUT_SECONDS\t3600\n'
printf '%s\n' RLT_STAGE2_RESOURCE_SMOKE_8ENV_PREPARED
