...
printf 'STARTED_AT\t%s\n' "$(cat "$runtime_root/started_at.txt" 2>/dev/null || printf 'MISSING')"
...
grep -aE 'Evaluating Rollout Epochs|Rollout Epochs|global_step|online_rows|total_online_rows|actor_updates|critic_updates' \
  "$runtime_root/driver.log" 2>/dev/null | tail -n 40 || true
printf '%s\n' '=== metrics tail ==='
find "$run_root" -name metrics.log -type f -print -exec tail -n 12 {} \; 2>/dev/null || true
printf '%s\n' '=== exception hints ==='
...
printf '%s\n' '=== driver tail ==='
tail -n 40 "$runtime_root/driver.log" 2>/dev/null || true
