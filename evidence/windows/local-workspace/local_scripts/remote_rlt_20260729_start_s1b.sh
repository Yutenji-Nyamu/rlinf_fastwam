...
RUNTIME_ROOT="$EVIDENCE_ROOT/s1b_runtime"
...
test ! -e "$RUNTIME_ROOT"
if pgrep -af 'train_vla_sft|ray::|raylet|gcs_server' | grep -v -E 'pgrep -af|start_s1b' >/dev/null; then
  printf '%s\n' 'refusing to start S1-B: relevant process already exists'
