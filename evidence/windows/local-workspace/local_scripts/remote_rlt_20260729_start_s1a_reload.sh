...
RELOAD_NAME=robotwin_adjust_bottle_rlt_stage1_s1a_reload_only_v1
CHECKPOINT_ROOT="$RUN_ROOT/s1a/$S1A_NAME/checkpoints/global_step_2"
RUNTIME_ROOT="$EVIDENCE_ROOT/s1a_reload_runtime"
...
test ! -e "$RUN_ROOT/s1a_reload/$RELOAD_NAME"
test ! -e "$RUNTIME_ROOT"
if pgrep -af 'train_vla_sft|ray::|raylet|gcs_server' | grep -v -E 'pgrep -af|start_s1a_reload' >/dev/null; then
  printf '%s\n' 'refusing to start reload: relevant process already exists'
...
timeout --signal=TERM --kill-after=60s 1800s \
  "$PYTHON_BIN" -B examples/sft/train_vla_sft.py \
    ...
    "runner.logger.log_path=$RUN_ROOT/s1a_reload" \
    "runner.logger.experiment_name=$RELOAD_NAME" \
    "+runner.resume_dir=$CHECKPOINT_ROOT"
rc=\$?
...
printf 'DRIVER_PID=%s\nMONITOR_PID=%s\nCHECKPOINT_ROOT=%s\nRUNTIME_ROOT=%s\n' \
  "$driver_pid" "$monitor_pid" "$CHECKPOINT_ROOT" "$RUNTIME_ROOT"
