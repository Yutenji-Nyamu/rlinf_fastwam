"""Fetch compact live metrics and logs for the current SZ pi0.5/Fast-WAM pair."""
...
DEST = (
    ...
    / "evidence"
    / "current-pair-live-20260902-1552"
)
RUNS = {
    "pi05_control": (
...
def remote_program() -> str:
    run_map = {label: str(path) for label, path in RUNS.items()}
    return f'''\
import json
from pathlib import Path
...
def download_file(sftp, remote: str, local: Path) -> None:
...
def main() -> None:
    ...
    try:
        ...
        if status != 0:
            raise RuntimeError(f"remote metric export failed ({status}): {error}")
        ...
        with client.open_sftp() as sftp:
            for label, run in RUNS.items():
                runtime = f"{run}/runtime"
                for name in ("driver.log", "resource.csv", "resolved.yaml", "exit_code", "timeout_exit_code"):
                    remote = f"{runtime}/{name}"
                    try:
                        download_file(sftp, remote, args.dest / "raw" / label / name)
