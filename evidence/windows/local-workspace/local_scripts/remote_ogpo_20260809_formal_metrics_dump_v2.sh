...
tb_root=/root/autodl-tmp/experiments/ogpo_robotwin_formal_20260808_v2/tensorboard
...
for tag in sorted(acc.Tags().get("scalars", [])):
    if not (tag.startswith("train/ogpo/") or tag.startswith("env/") or tag.startswith("eval/") or tag.startswith("time/")):
        continue
