...
最新只读刷新（09-06 10:18）：GPU6 BC完整45/100、fixed45=20/32，GPU7 DVAC43/100、fixed40=19/32；GRPO156/200、fixed155=21/32。三项继续、无所查fatal/OOM，ckpt40/40/150必要文件在（未恢复测试）。[本次采集＋fixed图、种子/耗时调查](evidence/PI05_BC_SEEDS_TIMING_CONTEXT_STORAGE_DISCUSSION_20260906.md)：环境seed/表/顺序相同，但独立rollout动作噪声未显式设种子；不把actor1234当全pipeline控制。普通轮约13min；环境RAM增长仍未定位。当前只讨论后续补齐RNG协议，不中途改run，也未清理/push。本页以下启动与smoke快照保留历史时间，不作当前现场。

π0.5 DVAC后续（09-06 00:03）：用户新增GPU7直接正式授权并明确不smoke，方法增量/范围[0.5,1.5]只维护在[DVAC设计§12](01_DVAC_DESIGN.md#12-π05-bc上复用dvacgpu7直接正式)及其执行账本。GPU6本文件的普通BC基线不改，不在这里复制第二套DVAC计划。
...
23:44真实driver命令/三worker GPU6、原模型和新run路径已核对，未加载Fast shim；23:46仅轻量证据提交`6a93605d`已push、clean，运行source-head912bc690和生产653fe0fb不变。完整JSON config未从driver文本独立解析出；逐叶配置依据为服务器Hydra compose＋相同源码/CLI/env核验，不夸大日志解析验证。按用户要求停止主动检查，不等待首轮完成、不新增自动化。
