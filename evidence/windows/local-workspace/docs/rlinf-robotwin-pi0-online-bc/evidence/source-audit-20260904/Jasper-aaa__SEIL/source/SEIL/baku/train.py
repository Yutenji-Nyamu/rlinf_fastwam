...
class WorkspaceIL:
    def __init__(self, cfg):
        ...
        if self.use_ema:
            print("#######################")
            print("###Traning EMA Model###")
            print("#######################")
            model_keys = ["actor", "encoder"]
            if self.agent.obs_type == "pixels" and self.agent.use_proprio:
                model_keys += ["proprio_projector"] 
            
            if self.agent.use_language:
                model_keys += ["language_projector"]

            self.ema_model_keys = model_keys
            # 未保持一致，并没有加入seperate encoder的情况
            self.ema_models = {
                k: copy.deepcopy(self.agent.__dict__[k]) for k in self.ema_model_keys
            }
    ...
    def eval(self):
        ...
        result_dict = {
            ...
            # "episode_length": step * self.cfg.suite.action_repeat / episode,
            # "episode": self.global_episode,
            # "step": self.global_step
        }    
        with open(f"{self.work_dir}/results_{self.global_step}.txt", "w") as file:
    ...
    def train(self):
        ...
        while train_until_step(self.global_step):
            ...
            # ema update
            if self.use_ema:
                for k in self.ema_model_keys:
                    utils.ema_update(self.ema_models[k], self.agent.__dict__[k], decay=0.995)
    ...
    def save_ema_snapshot(self):
        snapshot_dir = self.work_dir / "ema_snapshot"
        snapshot_dir.mkdir(exist_ok=True)
        snapshot = snapshot_dir / f"{self.global_step}.pt"
        payload = {}
        payload.update(self.ema_models)
        with snapshot.open("wb") as f:
            torch.save(payload, f)
