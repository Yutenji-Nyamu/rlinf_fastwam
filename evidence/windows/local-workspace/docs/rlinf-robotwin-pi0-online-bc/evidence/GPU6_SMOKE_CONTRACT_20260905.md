...
**结束状态（13:39补充）**：下方v1—v6合同均为已执行历史。v6于13:20:25 exit255：29/32成功、87query、2次Adam更新后首次评估相机buffer分配失败；GPU峰值79.17/79.65GiB，完整评估/checkpoint为0。无活跃BC，正式没有启动；下一项资源选择尚待确认，不能直接运行本合同旧命令。见[结果与接续§3](BC_RESULT_AND_NEXT_RESOURCE_CHOICE_20260905.md#3-下一步建议保持32并行环境按阶段释放)。

**当前执行v6（13:04更新）**：v5跨过64微批/2次更新，首次评估Vulkan导出失败。32train＋32eval隔离环境probe确认1024 FD耗尽（Errno24）。v6仅给本任务EnvWorker显式`env.min_open_files:4096`，不改变shared Ray或hard上限，已有更高/无限soft不降低。原32train/32eval并行、M4、MB32/GB1024/U2、2轮、fixed32每轮/save每轮、画面与种子全部不变。输出为`/data/chenyiteng/results/rlinf-shenzhen/online-bc/pi0-adjust-bottle-smoke32x1-m4-gpu6-20260905-v6`。服务器10项测试通过，完整resolved随本轮验证更新。正式仍先验收v6，再从原SFT/空池启动。下方v1—v5是故障追溯，不作活跃run。
...
历史v4：恢复官方DAgger的`precision:null`及对应FSDP null，保留OpenPI原生BF16主干/FP32投影，不再强制全模型入口BF16。用户最新明确要求关闭BC图像增强，故`openpi.image_augmentation:false`；仍保留resize/像素与动作归一化。采样32×1、M4、MB32/GB1024/U2及固定评估不变。先用v3已有成功数据独立验证一次真实FSDP更新，不采集、不保存或复用诊断权重，再从原SFT启动v4容量smoke。
