# AutoDL / RLinf 交接快照（2026-08-07 精简前）

最后更新：2026-08-07。本文只保留并行专题路由、当前停点和授权边界；旧时间线见
...
| 专题 | 唯一事实源 | 实施账本 | 当前停点 |
|---|---|---|---|
| OGPO × π0 × RoboTwin × RLinf | `docs/rlinf-robotwin-pi0-ogpo/00_INDEX_AND_IMPLEMENTATION_PLAN.md` | `docs/rlinf-robotwin-pi0-ogpo/evidence/CONTEXT_INVENTORY_LOG.md` | 服务器只读 freeze 已完成；只做 OGPO+CA、H50/C10 primitive replay 方案已收敛；未授权实现 |
...
| QAM × π0 × RoboTwin | `docs/rlinf-robotwin-pi0-qam/00_INDEX_AND_IMPLEMENTATION_PLAN.md` | `docs/rlinf-robotwin-pi0-qam/evidence/IMPLEMENTATION_LOG.md` | 历史停点（2026-08-01）：用户在 cycle 247 停止；最新完整 DCP 为 step 225，未做 held-out 对照 |
...
- 唯一当前入口为 `docs/rlinf-robotwin-pi0-ogpo/00_INDEX_AND_IMPLEMENTATION_PLAN.md`；详细来源职责与调用/数据流分别见同目录 `01_REFERENCE_MATRIX.md`、`02_CALL_AND_DATA_FLOW.md`。
- 2026-08-05 锁定 OGPO、RLinf、OpenPI、RoboTwin 在线参考 commit；官方 OGPO 是 JAX/Flax 独立 flow actor，没有现成 OpenPI π0/RoboTwin backend，因此算法 oracle 与目标系统骨架分开维护。
- 2026-08-07 已获授权完成服务器只读检查：当时容器 `autodl-container-nekaqbwt43-6ce5babb`；两张 A800-80GB 空闲，RAM available 约 982 GiB，数据盘可用约 816 GiB，无训练/Ray driver。
- 首版可执行父 commit 为 `/root/autodl-tmp/RLinf@6d0db56b`。公共 worktree 含用户未跟踪文件，不直接修改；DSRL/QAM/RLT worktree 当前干净，但只作窄参考。拟建独立 `/root/autodl-tmp/RLinf_ogpo_pi0_robotwin`、branch `codex/ogpo-pi0-robotwin`，尚未创建。
- 当前主线只实现 OGPO+CA，不提供 vanilla/OGPO+ runtime；`use_success_buffer_q=false`、`BoN=1`。π0 使用 frozen VLM + full action expert/projection + EMA expert，关闭 value head；critic 是额外的 10-head FP32 Q ensemble，只看 deployable frozen prefix feature + proprio + canonical action。
- action 方案为用户确认的 `H_model=50,C=10`：OGPO-only EnvWorker 逐 waypoint trace，actor 收到现有 rollout trajectory 后批量写 primitive replay，并组成真实 h-step sequence；使用 `Σ gamma^i r_i + gamma^h Q`，与官方 primitive TD 结构对齐。whole-chain PPO 评分完整 raw `[H50,D32]` 生成链，Q/环境只使用 `[C10,D14]` 前缀。K=4、G=32、UTD-Q/PI=1，Q target aggregation=mean。
- 目标 SFT 模型约 7.6 GiB、RoboTwin norm 已现场核验；复用 `/root/autodl-tmp/RLinf/.venv`（Python 3.11.14、Torch 2.6.0+cu124、Ray 2.55.1，JAX/Flax 已有），不复制 venv、不先安装 OGPO 依赖。
- 当前新增授权仅含服务器只读检查；未授权创建 worktree、代码实现、依赖/模型下载、项目测试、smoke 或训练。实施先做最小主路径和四组集中检查：OGPO 数学、RoboTwin primitive trace、一次 opt-in 集成更新、一次 save/load；G=32 的 microbatch 由小规模资源检查确定。
- 用户当前说明服务器已关闭；2026-08-07 较早资源/进程数据仅作快照，开机后再刷新，当前只讨论与维护文档。
