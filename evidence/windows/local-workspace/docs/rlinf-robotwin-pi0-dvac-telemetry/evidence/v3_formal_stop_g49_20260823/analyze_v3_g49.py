...
def plot_training(plot, metrics: pd.DataFrame, latest: int) -> None:
    ...
    image.save(OUT / "V3_FOUR_RUN_TRAINING_G49.png", optimize=True)
...
def plot_method(plot, dvac: pd.DataFrame, horizon: pd.DataFrame, latest: int, method: dict[str, float]) -> None:
    ...
    image.save(OUT / "V3_METHOD_DIAGNOSTICS_G49.png", optimize=True)
...
def plot_resources(plot, resources: pd.DataFrame) -> None:
    ...
    image, draw = plot.make_canvas(
        "DVAC v3 final resource telemetry through Global Step 49",
        "The run stopped after complete g49 without OOM; cgroup memory had touched the 240 GiB ceiling during training.",
        1120,
    )
    ...
    image.save(OUT / "V3_RESOURCES_G49.png", optimize=True)


def main() -> None:
    base = load_module("v3_g49_base", BASE_ANALYSIS)
    g2 = load_module("v3_g49_helpers", G2_ANALYSIS)
    plot = load_module("v3_g49_plot", PLOT_ANALYSIS)
    OUT.mkdir(parents=True, exist_ok=True)

    main_metrics = base.parse_metrics(RAW / "run/metrics.log")
    latest = int(main_metrics.global_step.max())
    if latest != 49:
        raise RuntimeError(f"snapshot contract expected g49, parsed g{latest}")
    ...
    npz_paths = sorted((RAW / "run/dvac_train").glob("actor_rank*/rollout_step0048.npz"))
    ...
    metrics.to_csv(OUT / "FOUR_RUN_TRAINING_G49.csv", index=False)
    rank.to_csv(OUT / "V3_DVAC_RANK_METRICS_G49.csv", index=False)
    dvac.to_csv(OUT / "V3_DVAC_STEP_METRICS_G49.csv", index=False)
    horizon.to_csv(OUT / "V3_LATEST_HORIZON_G49.csv", index=False)
    resources.to_csv(OUT / "V3_RESOURCES_G49.csv", index=False)
    ...
    summary = {
        ...
        "four_run_training_g1_to_g49": run_summaries,
        ...
        "v3_method_g49": method,
        "v3_method_apply_g2_to_g49": {
            ...
            "raw_log_v_geometric_change_g1_to_g49": float(math.exp(dvac.iloc[-1].current_mean - dvac.iloc[0].current_mean) - 1.0),
        },
        "resources": {
    ...
    (OUT / "SUMMARY_G49.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
