...
def load_base():
    spec = importlib.util.spec_from_file_location("g23_plot_base_for_g50", BASE_SCRIPT)
...
def plot_success(runs: dict[str, tuple[pd.DataFrame, str]], latest: int) -> None:
    ...
    image.save(OUT / "THREE_RUN_SUCCESS_G50.png", optimize=True)


def plot_optimization(runs: dict[str, tuple[pd.DataFrame, str]]) -> None:
    image, draw = make_canvas(
        "Three-run optimization metrics",
        "Same training protocol; v1 ends at g54, v2 snapshot ends at g50, original GRPO ends at g100.",
        1120,
    )
    specs = [
    ...
    image.save(OUT / "THREE_RUN_OPTIMIZATION_G50.png", optimize=True)
...
def plot_method(dvac: pd.DataFrame, horizon: pd.DataFrame, latest: int) -> None:
    ...
    image.save(OUT / "V2_METHOD_DIAGNOSTICS_G50.png", optimize=True)
...
def plot_resources(resources: pd.DataFrame) -> None:
    ...
    image.save(OUT / "V2_RESOURCES_G50.png", optimize=True)
...
def summarize_runs(runs: dict[str, tuple[pd.DataFrame, str]], latest: int) -> dict[str, object]:
    ...
    for label, (frame, _) in runs.items():
        ...
        result["runs"][label] = {
            ...
            "common_g1_to_g50_success_mean_pct": float(overlap.success_pct.mean()),
    ...
    for label in ("DVAC v1 global-z", "DVAC v2 R-only"):
        ...
        result["deltas_vs_grpo_common_window"][label] = {
            "g1_to_g50_mean_success_pp": row["common_g1_to_g50_success_mean_pct"] - baseline["common_g1_to_g50_success_mean_pct"],
...
def main() -> None:
    ...
    if latest != 50:
        raise RuntimeError(f"snapshot contract expected g50, parsed g{latest}")

    rank_paths = sorted((RAW / "run/dvac_train").glob("actor_rank*/runner_step_metrics.csv"))
    npz_paths = sorted((RAW / "run/dvac_train").glob("actor_rank*/rollout_step0049.npz"))
    ...
    pd.DataFrame(rows).to_csv(OUT / "THREE_RUN_METRICS_G50.csv", index=False)
    rank.to_csv(OUT / "V2_DVAC_RANK_METRICS_G50.csv", index=False)
    dvac.to_csv(OUT / "V2_DVAC_STEP_METRICS_G50.csv", index=False)
    horizon.to_csv(OUT / "V2_LATEST_HORIZON_G50.csv", index=False)
    ...
    summary = {
        "snapshot": {
            "live_probe_time": "2026-08-22T21:12:19+08:00",
            "latest_complete_global_step": latest,
            "next_phase_at_detail_probe": "g50 complete; subsequent rollout active",
            "controllers": "wrapper/driver/observer alive",
            "checkpoints": ["global_step_10", "global_step_20", "global_step_30", "global_step_40"],
    ...
    (OUT / "SUMMARY_G50.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
