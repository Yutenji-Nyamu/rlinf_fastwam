...
def write_summary_json(step_df: pd.DataFrame, h_df: pd.DataFrame, control_df: pd.DataFrame):
    ...
    summary = {
        "snapshot_latest": {
            ...
            "semantic_note": (
                "runner_step is zero-based: rollout_step0022 and CSV runner_step=22 "
                "correspond to the completed UI metric table Global Step 23/100."
            ),
            "history_used_for_weights_internal_steps": latest[
                "history_runner_steps_internal"
            ],
            "post_update_rolling_window_internal_steps": "18;19;20;21;22",
        },
        "latest_metrics": {
...
def draw_main_figure(step_df, h_df, control_df):
    ...
    draw.text(
        (50, 78),
        "Paired actor ranks · runner_step 0–22 = completed Global Steps 1–23 · snapshot evidence only",
        fill=MUTED,
        font=FONT_AXIS,
    )
    ...
    draw_axes(draw, plot, (1, 23), (ymin, ymax), [1, 5, 10, 15, 20, 23], np.linspace(-4.5, -4.1, 5), "Global step", "mean log(V_L3)")
    line(draw, x, current, plot, (1, 23), (ymin, ymax), BLUE)
    line(draw, x[hmask], history[hmask], plot, (1, 23), (ymin, ymax), ORANGE)
    ...
    draw_axes(draw, plot, (1, 23), (ymin, ymax), [1, 5, 10, 15, 20, 23], yticks, "Global step", "median V (log10)", lambda v: f"1e{int(v)}")
    for _, values, color in series:
        line(draw, x, values, plot, (1, 23), (ymin, ymax), color)
    ...
    xlim, ylim = (1, 23), (0.78, 1.22)
    draw_axes(draw, plot, xlim, ylim, [1, 5, 10, 15, 20, 23], [0.8, 0.9, 1.0, 1.1, 1.2], "Global step", "weight")
    ...
    draw_axes(draw, plot, (2, 23), (0, max_y), [2, 5, 10, 15, 20, 23], np.linspace(0, max_y, 5), "Global step", "% clipped / weight points", lambda v: f"{v:.0f}")
    line(draw, xa, high, plot, (2, 23), (0, max_y), RED)
    line(draw, xa, low, plot, (2, 23), (0, max_y), CYAN)
    line(draw, xa, gap_adv, plot, (2, 23), (0, max_y), PURPLE)
    ...
    legend(draw, plot[0] + 10, plot[1] + 8, [("all queries, steps 2–23", BLUE), ("valid, steps 2–23", GREEN), ("valid, latest", ORANGE)])
    ...
    img.save(HERE / "DVAC_FORMAL_STEP23_METHOD_OVERVIEW.png", optimize=True)
