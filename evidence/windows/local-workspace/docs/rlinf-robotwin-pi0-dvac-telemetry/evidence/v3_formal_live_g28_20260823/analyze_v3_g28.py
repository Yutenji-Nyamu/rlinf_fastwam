...
def plot_training(plot, metrics: pd.DataFrame, latest: int) -> None:
    ...
    image.save(OUT / "V3_FOUR_RUN_TRAINING_G28.png", optimize=True)
...
def plot_method(plot, dvac: pd.DataFrame, horizon: pd.DataFrame, latest: int, method: dict[str, float]) -> None:
    ...
    image.save(OUT / "V3_METHOD_DIAGNOSTICS_G28.png", optimize=True)
...
def plot_resources(plot, resources: pd.DataFrame) -> None:
    ...
    image, draw = plot.make_canvas(
        "DVAC v3 live resource telemetry through Global Step 28",
    ...
    image.save(OUT / "V3_RESOURCES_G28.png", optimize=True)


def main() -> None:
    base = load_module("v3_g28_base", BASE_ANALYSIS)
    g2 = load_module("v3_g28_helpers", G2_ANALYSIS)
    plot = load_module("v3_g28_plot", PLOT_ANALYSIS)
    OUT.mkdir(parents=True, exist_ok=True)

    main_metrics = base.parse_metrics(RAW / "run/metrics.log")
    latest = int(main_metrics.global_step.max())
    if latest != 28:
        raise RuntimeError(f"snapshot contract expected g28, parsed g{latest}")
    ...
    npz_paths = sorted((RAW / "run/dvac_train").glob("actor_rank*/rollout_step0027.npz"))
    ...
    metrics.to_csv(OUT / "FOUR_RUN_TRAINING_G28.csv", index=False)
    rank.to_csv(OUT / "V3_DVAC_RANK_METRICS_G28.csv", index=False)
    dvac.to_csv(OUT / "V3_DVAC_STEP_METRICS_G28.csv", index=False)
    horizon.to_csv(OUT / "V3_LATEST_HORIZON_G28.csv", index=False)
    resources.to_csv(OUT / "V3_RESOURCES_G28.csv", index=False)
    ...
    summary = {
        ...
        "four_run_training_g1_to_g28": run_summaries,
        ...
        "v3_method_g28": method,
        "v3_method_apply_g2_to_g28": {
            ...
            "raw_log_v_geometric_change_g1_to_g28": float(math.exp(dvac.iloc[-1].current_mean - dvac.iloc[0].current_mean) - 1.0),
        },
        "resources": {
    ...
    (OUT / "SUMMARY_G28.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
