...
日期：2026-08-24  
机器：SZ-H100  
状态：RLT Stage 1 已完成；Stage 2 v3事故已收束，v4与DSRL继续在一套持久Ray上分卡并发。动态step以服务器现场为准。
...
| RLT Stage 2 v3（历史事故） | GPU 4--5 | 8 env；250 cycles；GB512/MB128；UTD5 | `formal-current-ar-stage2-8env250-20260824-v3` |
| RLT Stage 2 v4（当前恢复） | GPU 4--5 | 参数与v3相同；只加入optimizer初始化合同修复 | `formal-current-ar-stage2-8env250-20260824-v4-warmup-fix` |
| DSRL | GPU 6--7 | 4 env；200 cycles；GB256/MB64；UTD20；ring25k | `formal-dsrl-pi0-robotwin-2gpu4env-200c-20260824-v2` |

### 01:04:53 CST 早期启动快照（历史）
...
- v3完整到Step24，Step25 fixed-20后卡在首次full checkpoint；它已按owned PGID与exact namespace
  精确停止，DSRL和shared Ray不动。
- DSRL在v3卡住后继续推进，并在同一Ray上完整保存Step65/130；v3停止时DSRL又自然从Step160推进到161。
- A0、B、A′三个最小RLT checkpoint都在DSRL并发时完整保存，排除“双job本身必挂”。
- 已证实plural `build_optimizers()`漏掉singular路径已有的`warmup_optimizer_state()`；修复为1 file/+2 LOC，
  commit `8bbd0216113ec6eca8bbc67e79d9d642d48b8ed1`，没有改objective、采样或训练预算。
- A0原代码也能保存，因此不把empty optimizer写成v3间歇hang的唯一根因；v4的Step25完整保存是实践终验。
- v4已在同样fixed-20=`0/20`、`update_step=0`条件下写出完整Step25 checkpoint并继续到Step28；DSRL同时
  连续推进到Step180。因而“同一persistent Ray上两项训练分卡并发”和修复后的首次pre-update保存都已闭环。
...
| generic FSDP checkpoint合同 | 是 | 1 file/+2 LOC；plural optimizer构造期复用既有warmup helper；不改科学算法 |

## 5. Git 与云端状态

- RLT：`codex/sz-rlt-pi0-robotwin-ar@8bbd0216113ec6eca8bbc67e79d9d642d48b8ed1`，服务器 worktree
...
- 并发编排本身只涉及Windows管理脚本和实验输出路径override，算法0 LOC；后续checkpoint事故另产生一个
  独立的generic FSDP两行修复commit。文档和启动器由本地专题账管理。
...
早期并发指令、PID、namespace与失败目录见
[`evidence/IMPLEMENTATION_LEDGER.md`](evidence/IMPLEMENTATION_LEDGER.md)；v3精确停止、A/B、两行修复、push和v4
恢复见[`evidence/RLT_CHECKPOINT_AND_MULTI_JOB_INVESTIGATION_LEDGER_20260824.md`](evidence/RLT_CHECKPOINT_AND_MULTI_JOB_INVESTIGATION_LEDGER_20260824.md)。
