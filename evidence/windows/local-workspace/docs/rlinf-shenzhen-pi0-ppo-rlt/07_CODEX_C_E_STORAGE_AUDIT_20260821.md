...
- `C:` 当前约余 36.01 GiB，`E:` 约余 334.46 GiB。
- 当前进程仍为 `CODEX_HOME=E:\Codex\home`、`CODEX_SQLITE_HOME=E:\Codex\home`；sessions/SQLite 主状态
  继续落 E。`TEMP/TMP` 仍在 `C:\Users\86136\AppData\Local\Temp`，当前项目工作区也仍在 C。
- 本轮 RLinf source audit worktree 约16 MiB；深圳专题文档/evidence约228 MiB。新生成的 π0/Fast-WAM
  DVAC 教学总包约57.78 MiB，因此当前工作不构成 C 盘容量风险。
- 后续 GB 级 clone、视频全集或资料包可直接从 E 路径开始；模型、数据与 checkpoint 继续只留服务器
  `/data`。不在桌面应用运行期间手工搬 AppData、TEMP 或旧 `.codex`。
