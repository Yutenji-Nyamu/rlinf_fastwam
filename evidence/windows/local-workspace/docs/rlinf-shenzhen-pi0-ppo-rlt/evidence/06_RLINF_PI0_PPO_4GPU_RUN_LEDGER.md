...
## 9. RL-SZ-R4-006 — 21:20 CST：按用户决定停止正式 PPO

- 用户在审阅 Step1–46 曲线和资源后，决定用当前 PPO 结果切换到参数贴近的正式 GRPO，并明确要求取消
  GRPO smoke。由此授权停止本轮 exact owned PPO；未触碰其他用户或其他专题进程。
- stop command file：
  `local_scripts/remote_commands/shenzhen_rlinf_stop_owned_ppo_formal100_20260822.sh`，2,132 bytes，
  SHA256=`5b10a046df2da612ef9edb6653aa0dbed92e2a35db5f2976e2100b6d3dbfce5a`。
- 发送信号前逐项核对：`driver.pid=1375834`、进程属主为`chenyiteng`、PGID与PID同为`1375834`，完整
  cmdline精确指向本run的`ppo-pi0-robotwin/.../train_embodied_agent.py`及既定formal参数。
- 停止前训练已自然完整到`Global Step 47/100`。21:20:10 CST只向owned PGID发送SIGINT；120秒后driver
  仍alive，按脚本既定窄升级向同一owned PGID发送SIGTERM。随后driver退出；**没有发送SIGKILL**。
- 首个退出后快照中只剩4个已由PID1接管的`EnvWorker` zombie，主存可用量已由约200.9 GiB恢复到
  `935,636,816 KiB≈892.3 GiB`；约90秒后的管理员只读复核确认GPU compute process已全部消失，整机
  `MemAvailable≈1.948 TiB`。这说明PPO/Ray资源已释放，未遗留活动训练进程。
- 已完成的Step47标量保留；最新带fixed-64评估且可恢复的自然checkpoint仍是`global_step_40`。既有
  checkpoint、日志、TensorBoard和视频均未删除或覆盖。

### 终态标量与进程复核

- `2026-08-22 21:28:16 CST`只读command file：
  `local_scripts/remote_commands/shenzhen_ppo_formal100_terminal_summary_20260822.sh`，SHA-256
  `9dd69748a9023b455b8f7c2d5bce39ffc1914fc7beef87d8ebd1558f45c11051`；exit0。
- Step47 train success/return=`0.89453125`，KL=`0.014`、clip fraction=`0.060`、grad norm=`32.559`、
  critic explained variance=`0.442`；step/rollout/actor=`1558.0/1532.5/23.156s`，全部finite。
- driver末尾是完整Step47指标、进入Step48首个rollout epoch后收到上述预期SIGTERM；没有新训练异常堆栈。
  原PPO driver与已知Ray worker PID survivor=`0`，GPU4–7无compute app。只读点看到的新Ray core/GPU2
  worker属于随后启动的π0 fixed-16推理，不是PPO残留。
- checkpoint目录仍精确为`global_step_10/20/30/40`，没有删除、覆盖或不完整Step47 checkpoint。
