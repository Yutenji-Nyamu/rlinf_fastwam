...
- prepare脚本确认服务器仍为clean `d81267b5...`后，两次SFTP只把更新后的
  `HANDOFF.md`和本账本写入新建的`authority_docs_staging`；commit脚本的两路径
  allowlist与全部QA通过，形成：

  ```text
  2b8199d8ab2e7b110994fd3234bf7007196c3af9
  docs(rlt): clarify cross-machine Git authority
  2 files changed, 79 insertions(+)
  ```

  父提交精确为`d81267b5...`，服务器worktree clean、ahead4。随后从
  `9bb2dd78...`到`2b8199d8...`重新生成ahead4增量bundle：

  ```text
  /root/autodl-tmp/experiment_exports/rlt_git_sync_20260731_0957_v1/
    codex-rlt-ahead4.bundle
  bytes=72516
  sha256=8f3c59cf7438e33e3f176be7d38bbd7239c1f383782b58e64b0f5618f1301311
  ```

  下载后SHA一致；临时bare relay验证bundle、cloud-base祖先关系、旧`d81267b5`到新
  `2b8199d8`祖先关系和`git fsck --connectivity-only`均通过。旧Windows开发镜像仍未参与。
- 用户随后给出明确云端写授权：把这4个fast-forward提交推到
  `Yutenji-Nyamu/rlinf_fastwam`的`codex/rlt-pi0-robotwin`。实际按安全顺序尝试：
  1. 服务器短探针出现main HTTP200，但20秒`ls-remote`仍超时；有界脚本exit75，未push；
  2. Windows沙箱外GCM路径在启动前返回access denied；未读取凭据或push；
  3. 服务器直推给55秒硬上限，返回124；云端未变；
  4. GitHub连接器有blob/tree/commit/ref API，但不能指定原始author/committer时间，
     会改变SHA并让两端分叉，因此未调用任何连接器写操作；
  5. 服务器SSH只存在`authorized_keys2`和`known_hosts`，无GitHub私钥；严格
     `ssh.github.com:443`探针因host key未预置而失败，未写known_hosts或改remote；
  6. 本机`git credential-manager get`返回255、无可用凭据；未push；
  7. 已准备的严格`github.com:443`反向CONNECT隧道脚本没有运行，因为用户随后建议先用
     AutoDL官方网络加速。
- RLT历史网络手册只记录了2026-07-30默认直连成功，没有记录`/etc/network_turbo`；
  用户随后补充：前一日已在GitHub操作中试过AutoDL加速，正确惯例是需要GitHub时临时
  开启、操作后立即关闭。本轮在RLT专题对该惯例重新取得机器证据，因此使用
  `apply_patch`新增
  `local_scripts/remote_rlt_20260731_git_sync_network_turbo_push.sh`。脚本先锁定branch、
  clean、HEAD=`2b8199d8...`、remote-tracking=`9bb2dd78...`和ahead4；随后只在子shell：
  ...
  GIT_TERMINAL_PROMPT=0 timeout 40 git push personal \
    refs/heads/codex/rlt-pi0-robotwin:refs/heads/codex/rlt-pi0-robotwin
  ```

  具体代理端点不打印、不写文件；不修改shell配置、Git config或remote。实际
  `http_proxy/https_proxy=SET`、其余大小写HTTP/ALL proxy未设置；`ls-remote`精确读到
  `9bb2dd78...`，随后约6.5秒完成：

  ```text
  9bb2dd78..2b8199d8
  codex/rlt-pi0-robotwin -> codex/rlt-pi0-robotwin
  TURBO_PUSH_OK_HEAD=2b8199d8ab2e7b110994fd3234bf7007196c3af9
  ```

  子shell退出后代理环境消失。该结果把用户已有的“需要GitHub时临时开启AutoDL turbo、
  操作后关闭”经验正式补入RLT专题，并继续禁止持久化代理。
- 为让云端也持久保存本次同步结果与网络经验，最终只更新`HANDOFF.md`、本账本和
  `06_AUTODL_NETWORK_PLAYBOOK.md`三份文档；对应prepare/commit-push脚本分别锁定
  已同步parent、clean、三路径allowlist、文本QA、staged diff和一次性turbo。脚本本地QA
  第一次把`foreach {...}`直接接管道，PowerShell 5.1在执行任何检查前返回
  `EmptyPipeElement`；没有文件或服务器变化。改为先赋值`$rows`再格式化，两个脚本均
  `bash -n`通过、BOM=false、CR=0、trailing=0且final-newline=true。
