...
| 2026-07-30（用户补充，跨项目） | GitHub操作曾临时开启AutoDL加速 | 需要时开启，操作后关闭，不持久化 |
| 2026-07-31 10:35 | main HTTP200但smart-HTTP持续超时；一次性`/etc/network_turbo`子shell | `ls-remote`和ahead4 push约6.5秒完成 |

默认成功配置仍是：无 proxy 环境变量、Git `http.version`默认值、现有HTTPS remote。
因此不因为一次超时就安装代理、强制HTTP/1.1、改remote或重建clone。只有默认直连经
短探针和一次有界push连续失败后，才使用第4节的一次性AutoDL加速子shell。
...
仅在默认直连已经按第2、3节失败，且服务器提供可读`/etc/network_turbo`时使用。必须在
子shell中source，不打印具体proxy端点、不写`.bashrc`、不改Git config或remote：
...
(
  ...
  GIT_TERMINAL_PROMPT=0 timeout 40 git push personal \
    HEAD:refs/heads/codex/rlt-pi0-robotwin
)
```

在source前仍须锁定branch、clean、HEAD、remote HEAD和ahead/behind；`ls-remote`结果必须
等于批准的旧远端HEAD，才允许push。子shell退出后检查父shell没有遗留proxy环境，再复核
remote HEAD、left/right `0/0`和worktree clean。若加速路径也失败，本轮停止，不继续叠加
第三方镜像、持久代理或remote改写。
