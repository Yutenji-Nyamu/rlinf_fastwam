...
当前精确停点是：**fresh smoke 已通过；resume 曾按用户要求省略；历史100-cycle run永久
归类为pilot；8-env资源门exit0；获批的RoboTwin `adjust_bottle`
`8 env × 250 cycles` formal已250/250自然完成、exit0。**
...
从`global_step_250`继续约10小时的只读审查已完成：`runner.max_steps`是绝对总cycle，
按P4实测速度推荐候选总终点480，即新增230 cycles。训练语义可以不变，但现有formal
launcher绑定`resume_dir=null/max_steps=250`，不能直接复用；续训尚未启动，也尚未生成
新的resolved审批包。由于此前没有做真实save→load→continue，若后续获准，必须把同一正式
续训进程的首个完整cycle作为恢复连续性健康门。
