...
> 状态：2026-07-30 实现候选版 v19（Stage 1 artifact 已验收；Stage 2 fresh smoke
> 已通过，resume 按用户要求省略；历史100-cycle run已永久归类为pilot；source-aligned
> RoboTwin `8×250` formal已于250/250、exit0自然完成，最终20-seed deterministic eval
> 为18/20、P4 train为79.45%；运行期间EnvWorker/anon阶梯满足预设风险定义，但退出后
> worker内存已释放；最新现场由根`HANDOFF.md`与15号最终报告刷新）
...
当前精确停点是：**fresh smoke 已通过；resume 按用户要求省略；历史100-cycle run永久
归类为pilot；8-env资源门exit0；获批的RoboTwin `adjust_bottle`
`8 env × 250 cycles` fresh formal已启动并健康完成首个完整cycle。**
...
formal于2026-07-30 11:37:25+08:00启动；11:41:49只读快照确认driver/monitor
PID `154857/154858`存活，首个cycle完成8条各200步reference rollout和151条macro
transitions。此时warm-up未满足，`update_step=0`符合10k rows/rank合同；两卡约17.1GiB，
无OOM/OOM-kill/PSI。resolved SHA-256为
`586644cd69461016c1dd8c653da0eea12b01c61f2d0a9b4901654d90800f2a3e`。
训练继续留在服务器；本窗口停止主动轮询，下次由用户要求时再刷新动态现场。

100-cycle run启动前没有完成多轴规模审批，故仍永久降级为pilot；当前formal才使用完整
2,000-episode预算、约39.1k macro rows、约125.5k/62.8k critic/actor updates、
每25 cycles做20条固定评估并保存一次，共10个评估点与10个checkpoint。
...
当前资源门、resolved config、精确命令、路径与首周期见
