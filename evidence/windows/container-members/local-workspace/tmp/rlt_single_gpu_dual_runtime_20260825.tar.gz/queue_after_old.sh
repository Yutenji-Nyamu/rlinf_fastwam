...
setsid bash "$package_root/run_one.sh" 0 "$control_config" "$control_run" "$control_runtime" "$control_name" >"$control_runtime/foreground.log" 2>&1 < /dev/null &
control_pid=$!
setsid bash "$package_root/run_one.sh" 1 "$dvac_config" "$dvac_run" "$dvac_runtime" "$dvac_name" >"$dvac_runtime/foreground.log" 2>&1 < /dev/null &
dvac_pid=$!
...
setsid bash "$package_root/paired_resource_monitor.sh" "$control_runtime" "$dvac_runtime" "$control_pgid" "$dvac_pgid" "$queue_runtime/paired_resources.csv" >"$queue_runtime/monitor.log" 2>&1 < /dev/null &
