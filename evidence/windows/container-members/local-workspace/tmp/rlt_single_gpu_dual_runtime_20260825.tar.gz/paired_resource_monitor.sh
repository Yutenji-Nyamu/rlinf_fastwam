...
printf '%s\n' 'unix_time,cgroup_current_bytes,cgroup_high_bytes,cgroup_max_bytes,event_high,event_max,event_oom,event_oom_kill,anon_bytes,file_bytes,psi_some_avg10,psi_full_avg10,gpu0_used_mib,gpu0_util_pct,gpu1_used_mib,gpu1_util_pct,control_pgid_rss_kib,dvac_pgid_rss_kib,compute_process_count' >"$out"

stat_value() {
  awk -v key="$1" '$1==key {print $2}' /sys/fs/cgroup/memory.stat 2>/dev/null
}
event_value() {
  awk -v key="$1" '$1==key {print $2}' /sys/fs/cgroup/memory.events 2>/dev/null
}
pgid_rss() {
  ps -eo pgid=,rss= | awk -v target="$1" '$1==target {sum += $2} END {print sum+0}'
}
...
while true; do
  ...
  control_rss=$(pgid_rss "$control_pgid")
  dvac_rss=$(pgid_rss "$dvac_pgid")
  compute_count=$(nvidia-smi --query-compute-apps=pid --format=csv,noheader,nounits 2>/dev/null | awk 'NF {n++} END {print n+0}')
  printf '%s\n' "$now,$current,$high,$max,$event_high,$event_max,$event_oom,$event_oom_kill,$anon,$file,$psi_some,$psi_full,$gpu0_used,$gpu0_util,$gpu1_used,$gpu1_util,$control_rss,$dvac_rss,$compute_count" >>"$out"
  if [ -f "${control_runtime}/finished_at.txt" ] && [ -f "${dvac_runtime}/finished_at.txt" ]; then
    break
  fi
  sleep 2
done
