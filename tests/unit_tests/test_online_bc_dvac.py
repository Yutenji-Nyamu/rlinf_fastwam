...
@pytest.mark.parametrize("alpha", [0.25, 0.125])
def test_calibrate_from_past_rounds_freeze_weights_bounds_and_restore(tmp_path, alpha):
    d = OnlineBCDvac(window=2, alpha=alpha)
...
def test_real_sampler_recording_preserves_actions_rng_and_forward_count(num_steps):
    ...
    def velocity(x, idx, *args):
        ...
        previews.append(x[:, :, :14] - (1 - idx / num_steps) * v[:, :, :14])
        return x - v / num_steps, torch.zeros_like(x), torch.zeros(x.shape[0]), v
