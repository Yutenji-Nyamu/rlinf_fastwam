...
def test_same_outcome_groups_receive_bounded_rloo_without_std(outcome):
    ...
    zero, _ = compute_prism_rloo_advantages(
        rewards, mask, 8, torch.full((8,), 0.5), 0.2
    )
...
def test_registry_uses_original_rewards_and_terminal_mask_with_chunk_loss():
    ...
    result = calculate_adv_and_returns(
        ...
        rewards=rewards,
        dones=dones,
        loss_mask=action_mask.any(-1, keepdim=True),
        loss_mask_sum=mask_sum[..., -1:],
        group_size=4,
...
@pytest.mark.parametrize(
    "case", ["empty_trajectory", "nonfinite_variance", "negative_variance"]
)
