...
def test_sampler_telemetry_preserves_actions_and_rng_sequence():
    ...
    for idx, velocity in enumerate(fake_with.velocities):
        expected = (
            with_telemetry["chains"][:, idx] - telemetry["timesteps"][idx] * velocity
        )[..., :2]
        torch.testing.assert_close(telemetry["z_endpoint"][:, idx], expected)


def test_current_openpi_signature_keeps_rtc_and_default_off_telemetry():
    signature = inspect.signature(OpenPi0ForRLActionPrediction.predict_action_batch)
    assert "rtc_context" in signature.parameters
    assert "return_dvac_telemetry" in signature.parameters
    assert signature.parameters["return_dvac_telemetry"].default is False


def test_rollout_merge_preserves_query_metadata_alignment():
    worker = object.__new__(MultiStepRolloutWorker)
    batches = [
        {
            "obs": {"states": torch.tensor([[1.0]]), "task_descriptions": ["a"]},
            "final_obs": None,
            "dvac_query_metadata": {
                "episode_idx": torch.tensor([10]),
                "video_relpath": ["seed_0/0.mp4"],
            },
        },
        {
            "obs": {"states": torch.tensor([[2.0]]), "task_descriptions": ["b"]},
            "final_obs": None,
            "dvac_query_metadata": {
                "episode_idx": torch.tensor([11]),
                "video_relpath": ["seed_1/0.mp4"],
            },
        },
    ]
    merged = worker._merge_obs_batches(batches)
    torch.testing.assert_close(merged["obs"]["states"], torch.tensor([[1.0], [2.0]]))
    torch.testing.assert_close(
        merged["dvac_query_metadata"]["episode_idx"], torch.tensor([10, 11])
    )
    assert merged["dvac_query_metadata"]["video_relpath"] == [
        "seed_0/0.mp4",
        "seed_1/0.mp4",
    ]
...
def test_rank_local_writers_preserve_query_and_episode_joins(tmp_path):
    ...
    writer.append(
        telemetry={
            ...
            "final_model_action": torch.full((batch_size, horizon, action_dim), 2.0),
        },
        ...
        env_obs={
            ...
            "wrist_images": torch.zeros(batch_size, 2, 4, 5, 3, dtype=torch.uint8),
        },
