...
def build_fastwam_policy(
    ...
    if bool(_cfg_get(cfg, "add_value_head", False)):
    ...
    rlt_cfg = _cfg_get(cfg, "rlt", {}) or {}
    if bool(_cfg_get(rlt_cfg, "enable", False)):
        from dataclasses import asdict

        from .rlt_policy import FastWAMRLTConfig, FastWAMRLTPolicy, feature_identity

        token_kwargs = dict(_cfg_get(rlt_cfg, "token_kwargs"))
        identity = feature_identity(checkpoint_path, stats_path, token_kwargs)
        ...
        if policy.stage1_steps < minimum_steps:
            raise ValueError(
                f"Stage1 has {policy.stage1_steps} updates; this run requires {minimum_steps}"
            )
    logger.info(
        "Built Fast-WAM RoboTwin policy | checkpoint=%s | stats=%s | "
        "trainable_parameters=%d | H/N/S=%d/%d/%d",
        checkpoint_path,
        stats_path,
        sum(parameter.requires_grad for parameter in policy.parameters()),
