...
class FastWAMPolicyConfig:
    ...
    value_feature_source: str = "video_cache_last_v_mean"
    value_feature_dim: int = 3072
...
class FastWAMPolicy(nn.Module, BasePolicy):
    ...
    def __init__(
        ...
        *,
        ...
        self.config = config
        # RLinf detects a critic through ``hasattr(policy, "value_head")``.
        # Do not register a placeholder for GRPO: PPO owns this top-level
        # module so optimizer value_lr, FSDP, sync and DCP use existing paths.
    ...
    def _validate_config(self) -> None:
        ...
        if cfg.value_feature_source != "video_cache_last_v_mean":
            raise ValueError(
                "Fast-WAM PPO requires video_cache_last_v_mean observation features"
            )
        if cfg.value_feature_dim != 3072:
            raise ValueError("Fast-WAM PPO observation feature width must be 3072")
        if not cfg.detach_critic_input:
            raise ValueError(
                "First-version Fast-WAM PPO requires detach_critic_input=true"
            )
    ...
    def _compute_values(self, observation_value_features: torch.Tensor) -> torch.Tensor:
        """Apply the PPO-only top-level head to frozen observation features."""

        if not hasattr(self, "value_head"):
            raise RuntimeError(
                "Fast-WAM value computation requires add_value_head=true"
            )
        if not torch.is_tensor(observation_value_features):
            raise TypeError("observation_value_features must be a Tensor")
        if observation_value_features.ndim != 2:
            raise ValueError("observation_value_features must be [B,D]")
        if observation_value_features.shape[1] != self.config.value_feature_dim:
            raise ValueError(
                "Fast-WAM observation value feature width mismatch: "
                f"{observation_value_features.shape[1]} != "
                f"{self.config.value_feature_dim}"
            )
        if not bool(torch.isfinite(observation_value_features).all()):
            raise ValueError("observation_value_features contains NaN/Inf")

        features = (
            observation_value_features.detach()
            if self.config.detach_critic_input
            else observation_value_features
        )
        ...
        if tuple(values.shape) != (features.shape[0], 1):
            raise ValueError(
                "Fast-WAM value_head must return [B,1], got " f"{tuple(values.shape)}"
            )
        values = values.float()
        if not bool(torch.isfinite(values).all()):
            raise ValueError("Fast-WAM value_head returned NaN/Inf")
        return values
    ...
    def predict_action_batch(
        ...
        **kwargs,
        ...
        calculate_values = (
            hasattr(self, "value_head")
            if compute_values is None
            else bool(compute_values)
        )
        if calculate_values and not hasattr(self, "value_head"):
            raise RuntimeError("compute_values=True requires add_value_head=true")
        ...
        for start in range(0, batch_size, chunk_size):
            ...
            conditioning = build_action_conditioning(
                ...
                include_observation_value_features=calculate_values,
                expected_value_feature_dim=(
                    self.config.value_feature_dim if calculate_values else None
                ),
            )
            if calculate_values:
                if conditioning.observation_value_features is None:
                    raise RuntimeError(
                        "Fast-WAM conditioning omitted requested value features"
                    )
                prev_values.append(
                    _cpu_detached(
                        self._compute_values(conditioning.observation_value_features),
                        dtype=torch.float32,
                    )
                )
        ...
        if deterministic:
            result = {}
            if calculate_values:
                result["prev_values"] = torch.cat(prev_values, dim=0)
        ...
        return executed_actions, {
            ...
            "prev_values": (
                torch.cat(prev_values, dim=0) if calculate_values else None
            ),
    ...
    def default_forward(
        ...
        **kwargs,
        ...
        if compute_values and not hasattr(self, "value_head"):
            raise RuntimeError("compute_values=True requires add_value_head=true")
        ...
        for start in range(0, batch_size, chunk_size):
            ...
            replay_output = recompute_logprob(
                ...
                noise_level=self.config.noise_level,
                tiled=self.config.tiled,
                return_observation_value_features=compute_values,
                expected_value_feature_dim=(
                    self.config.value_feature_dim if compute_values else None
                ),
            )
            if compute_values:
                logprob, entropy, observation_value_features = replay_output
                values.append(self._compute_values(observation_value_features))
            else:
        ...
        if compute_values:
            output["values"] = torch.cat(values, dim=0).float()
        return output
