...
"""RLT replay-BC controls adapted from the GRPO DVAC controls.

Annealing uses runner rounds; dropout uses the persisted replay update counter.
"""
...
def rlt_controls_contract(cfg: dict) -> dict:
    ...
    if contract and (
        cfg.get("factor_mapping", "linear_centered") not in {"linear_centered", "exp_mean"}
        or cfg.get("mapping") != "two_level_batch"
        or cfg.get("mode", "off") not in {"apply", "observe"}
    ):
        raise ValueError(
            "RLT DVAC controls require active two_level_batch and "
            "factor_mapping=linear_centered or exp_mean"
        )
    return contract
...
def effective_alphas(
    ...
    *,
    ...
    ``controls`` must be the result of :func:`rlt_controls_contract`.
    """
...
def apply_chunk_dropout(
    ...
    *,
    ...
    """Return selected replay chunks to BC weight one before microbatch splitting.

    One Bernoulli coin controls all H positions in each successful sampled query.
    Use the checkpointed critic/update counter: repeated calls at the same update
    are identical, but subsequent sampled batches receive different masks. The
    private CPU generator does not consume rollout, replay or model RNG streams.
    Failed chunks remain untouched; no renormalization or 1/(1-p) scaling.
    """
    if weights.ndim != 2 or not weights.is_floating_point():
        raise ValueError("weights must be floating [B,H]")
    if eligible_mask.shape != weights.shape[:1] or eligible_mask.dtype != torch.bool:
        raise ValueError("eligible_mask must be bool [B]")
    ...
    if probability == 1.0:
        ...
        selected = torch.rand(
            tuple(eligible.shape), generator=generator, device="cpu",
            dtype=torch.float32,
        ) < probability
        dropped = selected.to(device=weights.device) & eligible
    return torch.where(dropped[:, None], torch.ones_like(detached), detached).detach(), dropped
