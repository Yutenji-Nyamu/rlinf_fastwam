...
"""Executed-action DVAC quality for Prism-style trajectory RLOO."""
...
def trajectory_mean_log_variance(
    ...
    *,
    ...
    """Average log variance over the actions executed by each trajectory."""
    if variance.ndim != 3 or action_mask.shape != variance.shape:
        raise ValueError("variance and action_mask must both have shape [T,B,H]")
    ...
    if (counts == 0).any():
        raise ValueError("each trajectory must contain an executed action")
    log_variance = torch.log(variance.detach().float() + float(log_eps))
    return (log_variance * mask).sum(dim=(0, 2)) / counts


def reverse_rank_quality(cost: torch.Tensor, group_size: int) -> torch.Tensor:
    """Lower within-scene cost gets higher quality; exact ties share mid-rank."""
    if cost.ndim != 1:
        raise ValueError("cost must have shape [B]")
    if group_size < 2 or cost.numel() % group_size:
        raise ValueError("trajectory batch must be divisible by group_size >= 2")
    if not torch.isfinite(cost).all():
        raise ValueError("trajectory DVAC cost contains NaN or Inf")
    grouped = cost.detach().reshape(-1, group_size)
