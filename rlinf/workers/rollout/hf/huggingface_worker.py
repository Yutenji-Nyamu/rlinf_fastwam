...
class MultiStepRolloutWorker(Worker):
    def __init__(self, cfg: DictConfig):
        ...
        if self.dvac_telemetry_enabled:
            if not self.only_eval:
                raise ValueError(
                    "DVAC telemetry v1 only supports standalone evaluation."
                )
        ...
        if self.prism_dvac_enabled and self.dvac_train_enabled:
            if (
                self.algorithm_cfg.get("adv_type") != "prism_rloo"
                or self.dvac_train_cfg.get("normalization") != "two_level_group"
                or self.dvac_train_cfg.get("application")
                != "chunk_clipped_action_advantage"
            ):
                raise ValueError(
                    "Prism supports only the two-level DVAC new combination."
                )
            if self.prism_dvac_selected_l != self.dvac_selected_l:
                raise ValueError("Prism and DVAC new must share selected_l.")
            prism_eps = float(prism_cfg.get("log_eps", 1e-12))
            if not 0.0 < prism_eps < float("inf") or prism_eps != float(
                self.dvac_train_cfg.get("log_eps", 1e-12)
            ):
                raise ValueError(
                    "Prism and DVAC new must share a finite positive log_eps."
                )
    ...
    def init_worker(self):
        ...
        if self.dvac_telemetry_enabled:
            ...
            active_modes = [
                name
                for name in ("use_dsrl", "use_rlt", "is_nft", "rtc_enabled")
            ...
            self.dvac_telemetry_writer = DVACTelemetryWriter(
                ...
                run_metadata={
                    ...
                    "robotwin_commit": str(self.dvac_telemetry_cfg.robotwin_commit),
                    "checkpoint_revision": str(
                        self.dvac_telemetry_cfg.checkpoint_revision
                    ),
                    "norm_stats_sha256": str(self.dvac_telemetry_cfg.norm_stats_sha256),
                    "seed_file_path": str(self.cfg.env.eval.seeds_path),
                    "seed_file_sha256": str(self.dvac_telemetry_cfg.seed_file_sha256),
                    "launch_command": str(self.dvac_telemetry_cfg.launch_command),
                    ...
                    "active_action_dim": int(self.hf_model.config.action_env_dim),
                    "execution_chunk_length": int(self.model_cfg.num_action_chunks),
                    ...
                    "rollout_world_size": int(self.placement.get_world_size("rollout")),
                    "env_world_size": int(self.placement.get_world_size("env")),
                    "camera_mapping": {
                        "main_images": "head",
        ...
        if self.prism_dvac_enabled:
            if self.hf_model.config.action_horizon != self.hf_model.config.action_chunk:
                raise ValueError(
                    "Prism v1 requires the full model horizon to be executed."
                )
            if not 2 <= self.prism_dvac_selected_l <= self.hf_model.config.num_steps:
                raise ValueError(
                    "Prism selected_l exceeds the denoising endpoint trace."
                )
