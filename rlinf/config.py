...
def validate_prism_dvac_cfg(cfg: DictConfig) -> None:
    ...
    if (
        ...
        raise ValueError(
            "Prism preserves chunk-level rewards and chunk-level PPO clipping."
        )
    local_dvac_mode = str(
        OmegaConf.select(cfg, "algorithm.dvac_gradient_weighting.mode", default="off")
    ).lower()
    if local_dvac_mode != "off":
        dvac_cfg = cfg.algorithm.dvac_gradient_weighting
        if (
            local_dvac_mode != "apply"
            or dvac_cfg.get("normalization") != "two_level_group"
            or dvac_cfg.get("application") != "chunk_clipped_action_advantage"
        ):
            raise ValueError("Prism supports only the two-level DVAC new combination.")
        if int(dvac_cfg.get("selected_l", 3)) != int(prism_cfg.get("selected_l", 3)):
            raise ValueError("Prism and DVAC new must share selected_l.")
        if float(dvac_cfg.get("log_eps", 1e-12)) != float(
            prism_cfg.get("log_eps", 1e-12)
        ):
            raise ValueError("Prism and DVAC new must share log_eps.")
    ...
    if (
        ...
        raise ValueError(
            "Prism group size must match scene grouping and be at least two."
        )
    ...
    for model_cfg in (cfg.actor.model, cfg.rollout.model):
        ...
        if int(model_cfg.num_action_chunks) != int(model_cfg.openpi.action_chunk):
            raise ValueError(
                "Prism requires matching rollout and executed chunk lengths."
            )
