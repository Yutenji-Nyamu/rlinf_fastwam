...
ST = Path('/data/chenyiteng/results/server-maintenance-20260911/grpo-seed42-half')
...
ROOT = Path('/data/chenyiteng/projects/rlinf-shenzhen/worktrees/pi05-grpo-clean-half-seed42-20260911')
BASE_HEAD = '1d015a2aa03ec8132d8207ba47a2be3dbe1d9591'
BRANCH = 'codex/sz-pi05-grpo-clean-half-seed42-20260911'
RUN = Path('/data/chenyiteng/results/rlinf-shenzhen/pi05-sidney/runs/pi05-grpo-clean-half128-seed42-formal200-phys45-20260911-v1')
PRE = ST / 'prepared'
PY = '/home/chenyiteng/venvs/rlinf-7d07-openpi-robotwin/bin/python'
NAMESPACE = 'RLinf_grpo_clean_half128_seed42_formal45_20260911'
...
def prepare():
    ...
    experiment = 'pi05_grpo_clean_half128_seed42_formal200_phys45_20260911_v1'
    ...
    overrides = {
        ...
        'actor.group_name': 'ActorGroup_cleanhalfseed42_formal45',
        'rollout.group_name': 'RolloutGroup_cleanhalfseed42_formal45',
        'env.group_name': 'EnvGroup_cleanhalfseed42_formal45',
    ...
    unexpected = {key: value for key, value in changes.items()
                  if key not in identity | {'rollout.seed'}}
...
'''
    ...
    save(PRE / 'contract.json', {'time': now(), 'root': str(ROOT), 'run': str(RUN),
         ...
         'dvac': 'off', 'prism': 'absent', 'rollout_seed':42, 'logical_rank_seeds':[42,43], 'eval_rng_isolated':True,
...
def source_check():
    assert git(ROOT,'rev-parse','HEAD')==EXPECTED_HEAD and git(ROOT,'branch','--show-current')==BRANCH
    assert not git(ROOT,'status','--porcelain')
    receipt=json.loads((ST/'code-receipt.json').read_text())
    hashes={r['path']:r['sha256'] for r in receipt['files']}
    for rel in ['rlinf/config.py','rlinf/workers/actor/embodied_fsdp_actor_worker.py','rlinf/utils/utils.py','rlinf/runners/embodied_runner.py','rlinf/models/embodiment/openpi/openpi_action_model.py','examples/embodiment/config/robotwin_move_stapler_pad_grpo_openpi_pi05_sidney.yaml']:
        data=(ROOT/rel).read_bytes()
        assert data==subprocess.check_output(['git','-C',str(ROOT),'show',BASE_HEAD+':'+rel])
        hashes[rel]=hashlib.sha256(data).hexdigest()
    assert all(hashlib.sha256((ROOT/rel).read_bytes()).hexdigest()==sha for rel,sha in hashes.items())
    value={'time':now(),'head':EXPECTED_HEAD,'base':BASE_HEAD,'branch':BRANCH,'source_hashes':hashes,'production_changes':['rlinf/workers/rollout/hf/huggingface_worker.py'],'tests_passed':True,'cross_device_rng_passed':True}
