"""Exact-target DVAC new half128 replacement; preserve the seed42 Control budget."""
...
ST = Path('/data/chenyiteng/results/server-maintenance-20260912/grpo-dvac-new-half')
CLEAN_ROOT = Path('/data/chenyiteng/projects/rlinf-shenzhen/worktrees/pi05-grpo-clean-half-seed42-20260911')
CLEAN_RUN = Path('/data/chenyiteng/results/rlinf-shenzhen/pi05-sidney/runs/pi05-grpo-clean-half128-seed42-formal200-phys45-20260911-v1')
EXPECTED_HEAD = json.loads((ST/'code-receipt.json').read_text())['head']
BASE = Path('/data/chenyiteng/projects/rlinf-shenzhen/worktrees/sidney-pi05-current-rlinf')
ROOT = Path('/data/chenyiteng/projects/rlinf-shenzhen/worktrees/pi05-grpo-dvac-new-half-seed42-20260912')
BASE_HEAD = json.loads((ST/'code-receipt.json').read_text())['base']
BRANCH = 'codex/sz-pi05-grpo-dvac-new-half-seed42-20260912'
RUN = Path('/data/chenyiteng/results/rlinf-shenzhen/pi05-sidney/runs/pi05-grpo-dvac-new-half128-seed42-formal200-phys45-20260912-v1')
PRE = ST / 'prepared'
PY = '/home/chenyiteng/venvs/rlinf-7d07-openpi-robotwin/bin/python'
NAMESPACE = 'RLinf_dvac_new_half128_seed42_formal45_20260912'
CLEAN_NAMESPACE = 'RLinf_grpo_clean_half128_seed42_formal45_20260911'
...
def prepare():
    ...
    runtime={name:(CLEAN_RUN/name).read_text() for name in ('runtime/resolved.yaml','runtime/command.txt','runtime/environment.json','tensorboard/config.yaml')}
    oldcfg=yaml.safe_load(runtime['tensorboard/config.yaml'])
    runtime_diff = diff(yaml.safe_load(runtime['runtime/resolved.yaml']), oldcfg)
    allowed_runtime_defaults = {'runner.per_worker_log', 'runner.overlap_env_bootstrap', 'cluster.tracer.enable'}
    assert all(key in allowed_runtime_defaults and values == [None, False]
               for key, values in runtime_diff.items()), runtime_diff
    save(PRE/'baseline-runtime-defaults.json', runtime_diff)
    base={'runtime':{'runtime-resume100-to200/command.txt':runtime['runtime/command.txt'],'runtime-resume100-to200/environment.json':runtime['runtime/environment.json']}}
    ...
    experiment = 'pi05_grpo_dvac_new_half128_seed42_formal200_phys45_20260912_v1'
    ...
    overrides = {
        ...
        'actor.group_name': 'ActorGroup_dvacnewhalfseed42_formal45',
        'rollout.group_name': 'RolloutGroup_dvacnewhalfseed42_formal45',
        'env.group_name': 'EnvGroup_dvacnewhalfseed42_formal45',
    ...
    assert sum(token.lstrip('+').startswith('rollout.seed=') for token in tokens) == 1
    assert not any(token.lstrip('+').startswith('dvac_grpo=') for token in tokens)
    tokens.append('+dvac_grpo=adv_new')
    ...
    unexpected = {key: value for key, value in changes.items()
                  if key not in identity and not key.startswith('algorithm.dvac_gradient_weighting.')}
    ...
    assert method.mode == 'apply' and method.application == 'chunk_clipped_action_advantage'
    assert method.normalization == 'two_level_group' and method.scope == 'both'
    assert method.alpha_local == method.alpha_chunk == 1.0 and method.selected_l == 3
    assert method.log_eps == 1e-12 and method.minmax_eps == 1e-6
    assert method.save_step_tensors is True
...
'''
    ...
    save(PRE / 'contract.json', {'time': now(), 'root': str(ROOT), 'run': str(RUN),
         ...
         'dvac': OmegaConf.to_container(method, resolve=True), 'prism': 'absent', 'rollout_seed':42, 'logical_rank_seeds':[42,43], 'eval_rng_isolated':True,
...
def launch():
    stopped = json.loads((ST / 'stop-receipt.json').read_text())
    assert stopped['status'] == 'completed' and stopped['run'] == str(CLEAN_RUN)
    assert stopped['namespace'] == CLEAN_NAMESPACE
    ...
    assert shutil.disk_usage('/data/chenyiteng').free > 550 * 1024 ** 3
...
def source_check():
    ...
    receipt = json.loads((ST / 'code-receipt.json').read_text())
    hashes = receipt.get('source_hashes', {})
    if not hashes:
        hashes = {row['path']: row['sha256'] for row in receipt['files']}
    assert hashes and all(isinstance(key, str) and not key.startswith('/') and '..' not in Path(key).parts
                          for key in hashes)
    required = {'rlinf/workers/rollout/hf/huggingface_worker.py',
                'rlinf/workers/actor/embodied_fsdp_actor_worker.py',
                'rlinf/algorithms/dvac_two_level.py',
                'rlinf/algorithms/losses.py', 'rlinf/algorithms/utils.py',
                'examples/embodiment/config/dvac_grpo/adv_new.yaml'}
    assert required <= set(hashes), sorted(required - set(hashes))
    for relative, digest in hashes.items():
        assert hashlib.sha256((ROOT / relative).read_bytes()).hexdigest() == digest, relative
    assert git(ROOT, 'rev-parse', BASE_HEAD).startswith('76c2ac4c')
    # DVAC ports must retain the active control's rollout RNG implementation.
    rollout = 'rlinf/workers/rollout/hf/huggingface_worker.py'
    assert (ROOT / rollout).read_bytes() == subprocess.check_output(['git', '-C', str(ROOT), 'show', BASE_HEAD + ':' + rollout])
    value = {'time': now(), 'head': EXPECTED_HEAD, 'base': BASE_HEAD, 'branch': BRANCH,
             'source_hashes': hashes, 'tests': receipt.get('tests', receipt.get('tests_passed')),
             'production_changes': receipt.get('production_changes', receipt.get('files')),
             'rollout_rng_same_as_control': True}
...
if __name__ == '__main__':
    ...
    {'prepare': prepare, 'launch': launch, 'formal_driver': formal_driver, 'status': status}[sys.argv[1]]()
