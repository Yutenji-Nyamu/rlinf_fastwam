#!/usr/bin/env bash
set -u
run_dir=/data/chenyiteng/results/rlinf-shenzhen/online-bc/pi05-pillbottle-bc-dvac8x1-b1024-u2-m10-seed42-w0to2-eval8x4-gpu7-formal100-20260907-v1
...
timeout --signal=TERM --kill-after=180s 172800 "$venv/bin/python" -u \
  ...
  +online_bc_model=pi05_sidney +bc_dvac=default \
  +rollout.seed=42 \
  env.train.total_num_envs=8 algorithm.update_epoch=2 \
  runner.max_epochs=100 runner.val_check_interval=5 runner.save_interval=10 \
  actor.optim.total_training_steps=1000 \
  runner.logger.experiment_name=pi05-pillbottle-bc-dvac-seed42-w0to2-8x1-u2-formal100-gpu7-0907 \
