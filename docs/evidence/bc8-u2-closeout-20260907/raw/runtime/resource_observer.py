"""Bounded run-scoped GPU7 / host resource sampling; never controls jobs."""
import csv
import datetime
import json
import os
...
with (out/'resource.csv').open('x',newline='') as handle:
    ...
    writer.writerow(['timestamp','wrapper_alive','host_mem_available_kib','swap_free_kib','mem_psi_some_avg10','gpu7_used_mib','gpu7_util_pct','gpu7_temp_c','env_open_fds','env_rss_kib','actor_rss_kib','rollout_rss_kib'])
    while True:
        ...
        p=subprocess.run(['nvidia-smi','-i','7','--query-gpu=memory.used,utilization.gpu,temperature.gpu','--format=csv,noheader,nounits'],capture_output=True,text=True,timeout=10)
