#!/usr/bin/env bash
set -u
run_dir=/home/chenyiteng/results/rlinf-shenzhen/online-bc/pi05-pillbottle-bc-dvac8x1-b1024-u10-m10-seed42-w0to5-eval8x4-gpu7-formal100-20260907-v2
root=/data/chenyiteng/projects/rlinf-shenzhen/worktrees/pi05-online-bc-dvac-w5
...
timeout --signal=TERM --kill-after=180s 172800 "$venv/bin/python" -u \
  ...
  +online_bc_model=pi05_sidney +bc_dvac=bounded_zero_five \
  +rollout.seed=42 \
  env.train.total_num_envs=8 algorithm.update_epoch=10 \
  runner.max_epochs=100 runner.val_check_interval=5 runner.save_interval=10 \
  actor.optim.total_training_steps=1000 \
  runner.logger.experiment_name=pi05-pillbottle-bc-dvac-seed42-w0to5-8x1-u10-formal100-gpu7-0907-v2 \
